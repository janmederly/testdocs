...
Therefore tow new xref:/midpoint/reference/latest/expressions/expressions/script/functions/[Script Expression Functions] were added in midPoint 3.4 and 3.3.1: `composeDn`() and `composeDnWithSuffix()`:
...
The most likely symptom is that the xref:/midpoint/reference/latest/resources/shadow/[shadows] that represent the LDAP objects will duplicate.
