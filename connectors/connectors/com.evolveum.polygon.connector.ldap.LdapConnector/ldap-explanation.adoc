...
This is usually happens for reconciliation, import and similar xref:/midpoint/reference/latest/synchronization/introduction/[synchronization] tasks.
...
MidPoint will automatically discover xref:/midpoint/reference/latest/resources/resource-schema/[resource schema] by using the connector.
...
It has no xref:/midpoint/reference/latest/resources/resource-schema/[resource schema].
...
[NOTE]
====
MidPoint will retrieve LDAP schema once, and then it will cache it in the xref:/midpoint/reference/latest/resources/resource-configuration/[resource definition object]. Therefore is LDAP schema is changed or the configuration is switched to different LDAP server then midPoint will not see the updated schema.
In that case midPoint xref:/midpoint/reference/latest/resources/resource-schema/[resource schema] needs to be refreshed.
...
See xref:/midpoint/reference/latest/resources/auxiliary-object-classes/[Auxiliary Object Classes] page for more details.
...
MidPoint usually operates in a xref:/midpoint/reference/latest/concepts/relativity/[relative] way.
