...
This page describes troubleshooting of xref:..[LDAP Connector] and xref:/connectors/connectors/com.evolveum.polygon.connector.ldap.ad.AdLdapConnector/[Active Directory Connector (LDAP)]. Please make sure that you are familiar with xref:/midpoint/reference/latest/diag/troubleshooting/connectors/[basic connector troubleshooting techniques].
