...
Deploying a connector simply means copying the bundle (JAR file) to an appropriate place in xref:/midpoint/reference/latest/deployment/midpoint-home-directory/[midPoint home directory] and restarting midPoint.
...
Vast majority of the IDM logic is implemented in midPoint, such as xref:/midpoint/reference/latest/expressions/mappings/[Mappings], handling of resource capabilities, xref:/midpoint/reference/latest/synchronization/introduction/[Synchronization], etc.
...
This is used to implement xref:/midpoint/reference/latest/synchronization/introduction/[live synchronization]. This method will be invoked in short time intervals to check what has happened on the resource.
...
MidPoint will automatically recognize these attributes and use them in their proper place (e.g. as xref:/midpoint/reference/latest/concepts/activation/[activation properties] or credentials).
...
| `String`
| `PredefinedAttributeInfos`
| Set of xref:/midpoint/reference/latest/resources/auxiliary-object-classes/[auxiliary object classes] that the object has (see below).
...
The subtypes are translated to xref:/midpoint/reference/latest/concepts/matching-rules/[Matching Rules] in midPoint.
...
See xref:/midpoint/reference/latest/diag/troubleshooting/connectors/[Troubleshooting Connectors page] for more details about framework logging and log interpretation.
...
Please note that if there are *no paging options* at all then it is expected that the connector returns *all the objects*. This is important, because this is what the xref:/midpoint/reference/latest/synchronization/introduction/[reconciliation and import] do.
...
We refer to these additional object classes as xref:/midpoint/reference/latest/resources/auxiliary-object-classes/[auxiliary object classes].
...
Connector deployment is simple: just put it in appropriate subdirectory of xref:/midpoint/reference/latest/deployment/midpoint-home-directory/[midPoint home directory] and restart midPoint.
...
See also xref:/midpoint/reference/latest/diag/troubleshooting/connectors/[Troubleshooting Connectors] page for more details.
...
* xref:/midpoint/reference/latest/resources/connid/[] is aimed at connector developers, trying to use their new connector in midPoint.

* xref:/midpoint/reference/latest/resources/connector-setup/[] provides generic information on using stock connectors with midPoint.
