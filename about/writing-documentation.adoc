...
This document provides overview of how the _docs_ site works, and how to write and maintain Evolveum documentation. test2
...
If you want a narrow table, you must set the `auto-width` option in config to `false` and in adoc file you must define the table as `[%autowidth]`:
...
| `original-order`
| Preserve the original order of the rows at initialization, overrides `order` and `order-column` options
| false
| true, false
...
The layout is defined as JSON object.
For more details see https://datatables.net/reference/option/layout[DataTables documentation].
...
When using the `layout` option in adoc file, you must omit the logical conditions.
| JSON object
...
If disabled the width from asciidoc is used. So when you want to use narrow table you must define the table in asciidoc as `[%autowidth]` and set `auto-width` in config to `false`.
| true
| true, false
