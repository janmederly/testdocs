...
----
This layout is only available in javascript.
...
This option is different than auto-width in asciidoc tables.
If enabled, it calculates the column width based on the content of the table cells but the width is stuck at 100%.
If disabled the width from asciidoc is used. So when you want to use narrow table you must define the table is asciidoc as `[%autowidth]` and set `auto-width` in config to `false`.
| true
| true, false
