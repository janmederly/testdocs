...
When the process is stopped (e.g. administrator decision is required), midPoint will create a xref:/midpoint/reference/latest/cases/[case].
...
When user decision is needed to resolve a situation, midPoint synchronization service will create a xref:/midpoint/reference/latest/cases/[case].
