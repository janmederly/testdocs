...
The xref:/midpoint/reference/latest/synchronization/introduction/[synchronization] process treats these two cases in different manners.
More info on subject you will find xref:/midpoint/reference/latest/roles-policies/assignment/assigning-vs-linking/[here].
...
More on midPoints Role Based Access Control you will find xref:/midpoint/reference/latest/roles-policies/rbac/[here].
...
More on subject with example code you will find in chapter xref:/midpoint/reference/latest/resources/resource-configuration/[Resource Configuration].
...
These sections are more specifically described in xref:/midpoint/reference/latest/resources/resource-configuration/[Resource Configuration documentation].
...
You have to configure synchronization configuration (at least enable xref:/midpoint/reference/latest/resources/resource-configuration/synchronization/[synchronization], configure correlation/confirmation and inbound expressions).

A new task will be created named Import from resource (resource name) and you can check its status in xref:/midpoint/reference/latest/tasks/[] menu.
...
[TIP]
====
Consider this feature being functional only if xref:/midpoint/reference/latest/synchronization/introduction/[] part of Resource configuration is properly set.
====
...
* *stop scheduler + tasks*: the same as above, but, in addition, all tasks currently executing on that node are stopped (applying their `threadStopAction` attribute, see xref:/midpoint/reference/latest/tasks/task-manager/[Task Manager].
...
For more information please refer to the xref:/midpoint/reference/latest/tasks/task-manager/[Task Manager] section.
...
* *Tightly bound*: if yes, the task permanently occupies a thread on a node; if no, the thread is allocated when needed (and on any node); see xref:/midpoint/reference/latest/tasks/task-manager/[Task Manager]

* *Schedule interval*, *Schedule cron-line specification*, *Do not start before*, *Do not start after*: self-explaining; see also xref:/midpoint/reference/latest/tasks/task-manager/[Task Manager].

* *Misfire action*: what to do if the task could not be started on scheduled time: either execute immediately or reschedule; see xref:/midpoint/reference/latest/tasks/task-manager/[Task Manager].

* *Thread stop action*: what to do on node shutdown or failure: restart, reschedule, suspend, close; see xref:/midpoint/reference/latest/tasks/task-manager/[Task Manager].
...
Click the Cancel editing button to go back to xref:/midpoint/reference/latest/tasks/[#Server Tasks] and discard changes or Save button to save the changes.
...
Click the Save button to save the task, or click Back to go back to xref:/midpoint/reference/latest/tasks/[#Server Tasks].

=== Task Manager Configuration

Configuration of the task manager is (temporarily) described in the xref:/midpoint/reference/latest/tasks/task-manager/configuration/[Task Manager Configuration] document.
...
Please refer to the xref:/midpoint/reference/latest/security/audit/[Auditing] chapter for more information.
...
Please refer to the chapter xref:/midpoint/reference/latest/security/security-policy/[Security Policy Configuration] for more information.
...
Therefore there are xref:/midpoint/reference/latest/synchronization/flavors/[multiple ways] of creating user accounts in midPoint and/or linking them with their corresponding resource accounts.
...
The result of the correlation/confirmation expressions is a xref:/midpoint/reference/latest/synchronization/situations/[synchronization situation].
...
To configure Live synchronization, the resource has to support the cap:livesync xref:/midpoint/reference/latest/resources/resource-configuration/capabilities/[capability].
You also have to xref:/midpoint/reference/latest/synchronization/introduction/[configure] the correlation/confirmation expressions, the situation responses and actions and the inbound expressions for resource attributes.
...
To configure reconciliation, you have to xref:/midpoint/reference/latest/synchronization/introduction/[configure] the correlation/confirmation expressions, the situation responses and actions and the inbound expressions for resource attributes.
...
You have to enable xref:/midpoint/reference/latest/resources/resource-configuration/synchronization/[synchronization], the correlation/confirmation expressions, the situation responses and actions and the inbound expressions for resource attributes.
