...
NOTE: This is a historic design document. It was transformed into a xref:/midpoint/reference/latest/correlation/[feature description].
...
xref:/midpoint/reference/latest/resources/multiaccounts/[multi-accounts feature], allowing multiple
...
xref:/midpoint/reference/latest/schema/polystring-normalization/[configured `PolyString` normalization]).
|===
