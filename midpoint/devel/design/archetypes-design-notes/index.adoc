...
Planned feature description: xref:/midpoint/reference/latest/schema/archetypes/[Archetypes]
...
However, there is problem related to the way how midPoint xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[Clockwork and Projector] works.
...
All of that depends on the configuration of xref:/midpoint/reference/latest/admin-gui/collections-views/[views]. And there are several good reasons for that behavior:
...
We already have mechanism for explicit filter-based xref:/midpoint/reference/latest/admin-gui/collections-views/[explicit filter-based object collections]. However, it would not be convenient if we had to define explicit collection for each archetype.
...
The overlays specify object lifecycle state, xref:/midpoint/reference/latest/concepts/activation/[activation status] or operation (if used in buttons and links).
