...
. https://docs.evolveum.com/midpoint/reference/latest/admin-gui/admin-gui-config/#custom-actions-for-object-lists[Custom GUI actions for object lists] (that use task templates) should be callable by non-privileged users, even if they utilize Groovy code - bug:MID-6913[]/bug:MID-7831[].
