...
The issue is that rules dealing with segregation of duties (the `exclusion` policy constraint) are explicitly xref:/midpoint/reference/latest/roles-policies/segregation-of-duties/#_role_exclusion_definition[allowed] to be defined "uni-directionally", i.e. on the one of the two excluded roles.
