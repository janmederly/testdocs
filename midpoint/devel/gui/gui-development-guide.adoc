...
(SCSS, see xref:/midpoint/reference/latest/admin-gui/look-feel-customization/[Look & Feel Customization HOWTO])
...
* `WebModelServiceUtil`: contains utility methods that interact with midPoint xref:/midpoint/reference/latest/interfaces/model-java/[IDM Model Interface] (`ModelService`, `ModelInteractionService`) and other midPoint components.
