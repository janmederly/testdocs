...
// xref:/midpoint/reference/latest/repository/native-postgresql/postgresql-configuration/#db-server-sizing[]
...
With midPoint 4.4 we introduced the xref:/midpoint/reference/latest/repository/native-postgresql/[Native repository] that relies on PostgreSQL.
...
You may even consider xref:/midpoint/reference/latest/security/audit/#separate-repository-configuration-for-audit[separate database for audit]
...
* xref:/midpoint/reference/latest/deployment/ha/[High Availability and Load Balancing]