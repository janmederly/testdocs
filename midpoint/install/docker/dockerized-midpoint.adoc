...
For automatic schema creation and upgrade options please see xref:/midpoint/reference/latest/repository/generic/configuration/#schema-creation-and-updating[Schema creation and updating section in midPoint documentation]
(not available for xref:/midpoint/reference/latest/repository/native-postgresql/[Native PostgreSQL repository] yet).
...
Use for clustering.
For more information see xref:/midpoint/reference/latest/deployment/clustering-ha/[Clustering / high availability setup].
