...
[NOTE]
Do not confuse xref:/midpoint/reference/latest/deployment/stand-alone-deployment/#installation-directory[installation directory] that contains the binaries and scripts (normally extracted
from the distribution archive) with xref:/midpoint/reference/latest/deployment/midpoint-home-directory/[midPoint home directory] that is important for the running application, containing initial configuration and logs.
...
* xref:/midpoint/reference/latest/deployment/midpoint-home-directory/index.adoc[home directory]
** log
** xref:/midpoint/reference/latest/deployment/post-initial-import.adoc[_post initial objects_]
* class path / -jar
* JAVA_OPTS
...
Next to other JAVA environment or application parameter *-D** can be used to xref:/midpoint/reference/latest/deployment/midpoint-home-directory/overriding-config-xml-parameters.adoc[override config.xml parameters].
