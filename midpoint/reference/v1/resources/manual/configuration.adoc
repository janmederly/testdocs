...
See xref:/midpoint/reference/v1/resources/manual/[Manual Resource and ITSM Integration] for an introduction to the topic.

== Pure Manual Resources

Pure manual resources are configured as (almost) ordinary single-connector resources.
Internal manual connector implementation or an xref:/midpoint/reference/v1/resources/manual/itsm-plugin/[ITSM Plugin] should be used in place of an ordinary identity connector.
...
It is using the xref:/midpoint/reference/v1/resources/multi-connector-resource/[multi-connector resource] mechanism.
...
The deltas are kept in the xref:/midpoint/reference/v1/resources/shadow/[shadow objects]. When the operation is finished (case/ticket is closed) then the operation is applied to the cached values and the delta is not needed any more.
...
See xref:/midpoint/reference/v1/resources/manual/itsm-plugin/[ITSM Plugin] page for more details.
...
Therefore in that case midPoint relies on xref:/midpoint/reference/v1/resources/attribute-caching/[attribute caching]. Which means that the pending delta corresponding to the closed ticket is applied to the data cached in the shadow.
...
* xref:/midpoint/reference/v1/resources/multi-connector-resource/[Multi-Connector Resource]

* xref:/midpoint/reference/v1/resources/manual/itsm-plugin/[ITSM Plugin]