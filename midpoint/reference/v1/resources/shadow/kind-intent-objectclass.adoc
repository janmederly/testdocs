...
These terms define a type of resource object (or xref:/midpoint/reference/v1/resources/shadow/[shadow]). The _kind_ and _intent_ are midPoint terms.
...
Accounts can be _xref:/midpoint/reference/v1/roles-policies/assignment/assigning-vs-linking/[linked]_ to the xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/usertype/[User] objects.

* xref:/midpoint/reference/v1/resources/entitlements/[Entitlement] resource objects represent groupings or privileges of an *account*. Entitlement resource objects represent groups, resource-specific roles, access permissions or even organizational unit membership.
...
Tag is used in situations when xref:/midpoint/reference/v1/resources/multiaccounts/[multiple equivalent accounts] are enabled.
...
Kind, intent and tag are stored in xref:/midpoint/reference/v1/resources/shadow/[shadow objects]. Those are midPoint internal concepts.
...
The purpose of this discriminator is to distinguish xref:/midpoint/reference/v1/schema/focus-and-projections/[projections]. Each projection linked to a particular focus must have unique value of the discriminator.
...
Object classes are defined by the xref:/midpoint/reference/v1/resources/resource-schema/[Resource Schema] and are presented to midPoint by the connector.
...
The mapping on the outbound side is defined in xref:/midpoint/reference/v1/resources/resource-configuration/schema-handling/[Resource Schema Handling]. This maps (kind, intent) to object class.
...
The mapping on the inbound side is defined in xref:/midpoint/reference/v1/resources/resource-configuration/synchronization/[Synchronization Configuration]. This maps object class to (kind, intent) tuple.This mapping is not a very reliable and straightforward because all the midPoint really knows about the object is its object class and attributes.
...
NOTE: See also xref:/midpoint/reference/v1/resources/resource-configuration/schema-handling/changes-in-4.6.adoc[].
