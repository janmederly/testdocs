...
See also xref:/midpoint/reference/v1/simulation/tutorial/[] (since 4.7).
...
It is described in a xref:/midpoint/reference/v1/correlation/[separate document].
...
For each xref:/midpoint/reference/v1/synchronization/situations/[synchronization situation] there is a synchronization action specified.
...
This feature is new in 4.6, and is described in a xref:/midpoint/reference/v1/resources/resource-configuration/inheritance/[separate document].
...
See also xref:/midpoint/reference/v1/tasks/activities/resource-object-set-specification/[].
