...
Especially when entitlements are combined with the power of xref:/midpoint/reference/v1/roles-policies/assignment/[assignments] and xref:/midpoint/reference/v1/synchronization/generic-synchronization/[generic synchronization].
...
Similarly to all other resource-side object the entitlement is represented in midPoint only as a xref:/midpoint/reference/v1/resources/shadow/[shadow].
...
And that's exactly what midPoint does when an account xref:/midpoint/reference/v1/resources/shadow/[shadow] is retrieved:
...
Association of account and entitlement is defined in the xref:/midpoint/reference/v1/resources/resource-configuration/schema-handling/[Resource Schema Handling] section of xref:/midpoint/reference/v1/resources/resource-configuration/[Resource Configuration]. Both object-to-subject and subject-to-object association directions are supported
...
The associations are not stored in midPoint (xref:/midpoint/reference/v1/resources/shadow/[Shadow Objects]). The associations are retrieved fresh from the resource.
This applies to all parts of midPoint and particularly to xref:/midpoint/reference/v1/synchronization/introduction/[synchronization] and the user interface.
...
Entitlements can be easily assigned to accounts by using the xref:/midpoint/reference/v1/roles-policies/assignment/[assignment] mechanism.
This allows the construction of roles that automatically associate user's accounts with appropriate groups.
See xref:/midpoint/reference/v1/roles-policies/assignment/configuration/[Assignment Configuration] page for more details.
