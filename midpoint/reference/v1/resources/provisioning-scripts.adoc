...
====
Provisioning scripts are a different kind of animal than midPoint xref:/midpoint/reference/v1/expressions/expressions/script/[script expressions]. Provisioning scripts are usually executed in the target system and they are mostly outside midPoint control.
...
Argument specification may contain midPoint xref:/midpoint/reference/v1/expressions/expressions/script/[script expressions] that determine argument values.
...
For example there may be need to execute a provisioning script from xref:/midpoint/reference/v1/expressions/expressions/[expression] because the script is used to determine a value used by the expression.
For that purpose there is executeAdHocProvisioningScript function in xref:/midpoint/reference/v1/expressions/expressions/script/functions/midpoint/[MidPoint Script Library]:
...
====
MidPoint 3.0 (and later) is designed with xref:/midpoint/reference/v1/synchronization/generic-synchronization/[generic synchronization] in mind.
Therefore the provisioning scripts are executed not just for accounts but also for other xref:/midpoint/reference/v1/resources/shadow/kind-intent-objectclass/[kinds of objects]. And there are two major consequences of this:

* Please keep in mind that unless the script is limited to a particular xref:/midpoint/reference/v1/resources/shadow/kind-intent-objectclass/[kind] of objects then it will be executed for all object kinds that midPoint provisions.
