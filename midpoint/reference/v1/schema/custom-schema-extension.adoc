...
It is used in xref:/midpoint/reference/v1/expressions/[mappings] and configuration.
...
It is not stored in the xref:/midpoint/reference/v1/repository/[repository (database)]
and cannot be dynamically changed when the system is running.

The schema files have to be placed in the `schema` subdirectory of xref:/midpoint/reference/v1/deployment/midpoint-home-directory/[midpoint.home directory].
...
h| How is it stored in xref:/midpoint/reference/v1/repository/native-postgresql/[Native repository]?
h| How is it stored in xref:/midpoint/reference/v1/repository/generic/[Generic repository]?
...
See xref:/midpoint/reference/v1/repository/native-postgresql/db-maintenance/#index-tuning[Index tuning] for more information.
