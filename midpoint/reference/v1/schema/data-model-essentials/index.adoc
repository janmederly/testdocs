...
However, the entire point of midPoint is to (indirectly) xref:/midpoint/reference/v1/synchronization/introduction/[synchronize] the data in the midPoint database to other systems.
...
For the full description of the model please see xref:/midpoint/reference/v1/schema/[Data Model] page and the documentation inside the schema files (XSD).
====
...
| Role
| RoleType
| A role that implements xref:/midpoint/reference/v1/roles-policies/rbac/[Role-Based Access Control (RBAC)] mechanisms.
...
MidPoint resolves this difficulty by using xref:/midpoint/reference/v1/resources/shadow/[shadow objects] or "shadows" for short.
...
Significant part of midPoint functionality is controlled by xref:/midpoint/reference/v1/resources/resource-configuration/[configuration of the resource definition].
...
Main article: xref:/midpoint/reference/v1/roles-policies/rbac/[midPoint RBAC]
...
The xref:/midpoint/reference/v1/schema/[Data Model] page provides in-depth description of the data model design principles and purpose (although it may be slightly outdated).

The xref:/midpoint/reference/v1/schema/schemadoc/[SchemaDoc] page contains links to data model documentation that is automatically generated from schema files.
...
* xref:/midpoint/reference/v1/schema/schemadoc/[SchemaDoc]