...
It looks for objects whose xref:/midpoint/reference/v1/concepts/activation/[validity] is assumed to be changed
...
| `objects` | What xref:/midpoint/reference/v1/tasks/activities/object-set-specification/[objects] to scan. Normally, there is no need to specify the query. If it is present, the filter computed
...
xref:/midpoint/reference/v1/tasks/activities/distribution/[`distribution/subtasks`] value.
