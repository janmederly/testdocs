...
The usage and the structure for the ServiceType object is almost the same as xref:/midpoint/reference/v1/roles-policies/rbac/[midPoint roles] (xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/roletype/[RoleType]). It means, you can define rules for provisioning, you can assign roles, xref:/midpoint/reference/v1/roles-policies/metaroles/gensync/['meta-roles, meta-meta-roles'] to them etc.
...
If the standard attributes defined in the schema for ServiceType are not satisfactory, xref:/midpoint/reference/v1/schema/custom-schema-extension/[extension schema] can be used.
...
Service objects in midPoint act as xref:/midpoint/reference/v1/schema/focus-and-projections/[focal objects]. It is expected that xref:/midpoint/reference/v1/schema/focus-and-projections/[projections] of the service objects will be objects that represent the services on the resource.
In the above example the service foo.example.com is modeled as service object in midPoint.
When all the provisioning is done then the midPoint service object will have a projection (xref:/midpoint/reference/v1/resources/shadow/[shadow]) in the OpenStack resource.
...
The brief comparison which object type to use in which situation can be also find xref:/midpoint/reference/v1/roles-policies/roles-services-and-orgs/[here].
