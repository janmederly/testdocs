...
The keystore is normally stored in xref:/midpoint/reference/v1/deployment/midpoint-home-directory/[midPoint home directory] in a `keystore.jceks` file.
...
MidPoint has its own keystore in xref:/midpoint/reference/v1/deployment/midpoint-home-directory/[midPoint home directory] and this keystore is used for all operations that are under midPoint control.
