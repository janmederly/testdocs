...
We take care to select a good cipher (AES), generate random key for every midPoint deployment, store the key in the xref:/midpoint/reference/v1/security/crypto/keystore-configuration/[keystore] and not in the database and so on.
...
The storage mechanism can be configured in xref:/midpoint/reference/v1/security/security-policy/[security policy]:
...
Accounts that were created and were not yet initialized are marked in midPoint by using the xref:/midpoint/reference/v1/concepts/object-lifecycle/[lifecycle state property]. The uninitialized accounts are in the `proposed` lifecycle state.
...
This can also be configured in xref:/midpoint/reference/v1/security/security-policy/[security policy]:
...
* xref:/midpoint/reference/v1/security/crypto/[Encryption and Keys]
