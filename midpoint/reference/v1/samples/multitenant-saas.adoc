...
* organizationalUnit ou=ultra1,ou=customers,dc=example,dc=com will be created to replicate midPoint organizational structure (xref:/midpoint/reference/v1/synchronization/generic-synchronization/[Generic Synchronization])
...
. The account data are copied to the new user by xref:/midpoint/reference/v1/expressions/mappings/inbound-mapping/[inbound mappings] on CRM resource.
...
This meta-role contains `inducements` which specifies that a new `ou` and three groups should be created as a xref:/midpoint/reference/v1/schema/focus-and-projections/[projections] for each Org. +
...
Each inducement specifies a xref:/midpoint/reference/v1/resources/shadow/kind-intent-objectclass/['(kind, intent)'] tuple which is used to locate a matching definition in the schemaHandling.

.... The xref:/midpoint/reference/v1/expressions/mappings/outbound-mapping/[outbound mappings] are used to compute a correct DN for the new "ou" object in LDAP and all three "groupOfNames".
...
This just assigns the "Customer Admin Role" role `(role-customer-basic-admin.xml`) . This is a simple xref:/midpoint/reference/v1/roles-policies/rbac/[RBAC] role that assigns an LDAP account to the user.
...
This just assigns the "Delegated Administration Role" role `(role-customer-authz-admin.xml`) . This role assigns midPoint xref:/midpoint/reference/v1/security/authorization/[authorizations] only (no provisioning).
The role is assigned to midPoint users with `employeeType` attribute value "customer-admin".

.. The "Basic Customer User role assignment" mapping in user template is processed.
This just assigns the "Customer User Role" role `(role-customer-basic-user.xml`) . This is a simple xref:/midpoint/reference/v1/roles-policies/rbac/[RBAC] role that assigns an LDAP account to the user.
...
.. The "Customer User Role" assigns an LDAP account to the user and using xref:/midpoint/reference/v1/resources/entitlements/[entitlement association] adds user to "ultra1-users" group.
The group name is derived from user's `organizationalUnit` attribute value.
No other attributes are specified in the role.
The xref:/midpoint/reference/v1/expressions/mappings/outbound-mapping/[outbound mappings] from the OpenLDAP resource definition are used to fill in account attribute values.
