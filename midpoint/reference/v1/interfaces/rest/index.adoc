...
*Dependencies:* xref:/midpoint/reference/v1/schema/[Data Model], Common schema, Query schema +
...
In that case midPoint xref:/midpoint/reference/v1/synchronization/consistency/[consistency mechanism] may be triggered and it can modify the state of the midPoint objects.
...
Typical example is an execution of a xref:/midpoint/reference/v1/misc/bulk/[bulk action].
...
* 240 for handled error which means that there was originally error, but midPoint was able to fix this using its xref:/midpoint/reference/v1/synchronization/consistency/[consistency mechanism.]
...
| /\{type}/search
| link:https://github.com/Evolveum/midpoint/blob/731e408905650d81ecab410f1f1c4f07a0d8795c/infra/schema/src/main/java/com/evolveum/midpoint/schema/GetOperationOptions.java[GetOperationOptionsType]
| xref:/midpoint/reference/v1/concepts/query/[Query] in XML format
...
* xref:/midpoint/reference/v1/concepts/query/[midPoint Query]
