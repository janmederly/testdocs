...
When you look on the example above showing the usage of the function evaluator and the example of the function library object, you can see that the parameter name _location_ coincide with the name of the parameter _location_ in the function _getLocality_. The value used for the parameter itself is xref:/midpoint/reference/v1/expressions/expressions/[expression]. So you can use each expression evaluator here to get a value for the parameter.
