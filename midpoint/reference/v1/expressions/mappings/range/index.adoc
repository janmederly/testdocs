...
The usual way for automatic assignment of roles is use of xref:/midpoint/reference/v1/expressions/mappings/inbound-mapping/[inbound mapping] with xref:/midpoint/reference/v1/expressions/expressions/#assignment-target-search[assignmentTargetSearch] expression evaluator.
...
The mapping is processing the values of `project` attribute, creating assignments to appropriate xref:/midpoint/reference/v1/org/[orgs].
...
Such mappings are evaluated in different phases of xref:/midpoint/reference/v1/concepts/clockwork/clockwork-and-projector/[projector computation], which may lead to incorrect results.
