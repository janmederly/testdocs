...
Please note that explicit `stringify()` invocation is needed in this case as the `organizationalUnit` property is of xref:/midpoint/reference/v1/concepts/polystring/[PolyString] type.
As all midPoint mapping even this mapping is xref:/midpoint/reference/v1/concepts/relativity/[relativistic].
...
In fact the mapping is evaluated in exactly the same way as xref:/midpoint/reference/v1/expressions/object-template/[object template] mappings.
...
In that case it may be better to consider a more systemic approach by using xref:/midpoint/reference/v1/expressions/object-template/[object template].
...
* xref:/midpoint/reference/v1/roles-policies/role-autoassignment/[Role Autoassignment]
