...
MidPoint contains mechanisms that can be used to manage lifecycle of the roles and role-like objects (xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/roletype/[roles], xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/orgtype/[orgs], xref:/midpoint/reference/v1/misc/services/[services] and even xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/usertype/[users]). Lifecycle management means that midPoint can apply constraints and xref:/midpoint/reference/v1/roles-policies/policy-rules/[rules] to various lifecycle events such as creation of a new role, change of role definition, deletion and so on.
MidPoint can prohibit certain lifecycle transitions, require approval and so on.

The role lifecycle is implemented by combining xref:/midpoint/reference/v1/cases/approval/[approval] process with xref:/midpoint/reference/v1/roles-policies/policy-rules/[policy rules]. The policy rules are triggered when the role is created, modified or deleted.
The rules specify xref:/midpoint/reference/v1/cases/approval/[approval] as an action, therefore the change operation is suspended and it is driven through approval processes.
...
MidPoint contains a xref:/midpoint/reference/v1/concepts/object-lifecycle/[special property to control object lifecycle]: `lifecycleState`. This property specifies current lifecycle state of the object.
...
See xref:/midpoint/reference/v1/concepts/object-lifecycle/[Object Lifecycle] page for more details.
...
* xref:/midpoint/reference/v1/concepts/object-lifecycle/[Object Lifecycle]