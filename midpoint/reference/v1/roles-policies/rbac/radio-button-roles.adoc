...
. xref:/midpoint/reference/v1/roles-policies/policy-rules/[Policy Rules] that implement (almost) ordinary xref:/midpoint/reference/v1/roles-policies/segregation-of-duties/[role exclusion].

. The "prune" policy action.

. Clever use of xref:/midpoint/reference/v1/roles-policies/metaroles/gensync/[metarole] to define the radio button class.
