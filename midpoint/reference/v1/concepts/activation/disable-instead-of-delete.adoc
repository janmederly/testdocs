...
And this can be quite easily achieved by adjusting the xref:/midpoint/reference/v1/concepts/activation/[activation] xref:/midpoint/reference/v1/expressions/mappings/outbound-mapping/[outbound mappings].
...
Whether an account is created or deleted is determined by existence mapping in the activation part of xref:/midpoint/reference/v1/resources/resource-configuration/schema-handling/[Resource Schema Handling] definition.
...
That means if there is a valid assignment for that account or if the account is allowed by any other policy (such as xref:/midpoint/reference/v1/synchronization/projection-policy/[Projection Policy]). The `legal` variable is set to `false` if there is no "habeas corpus" for that account.
