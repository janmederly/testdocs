...
stored persistently in xref:/midpoint/reference/v1/repository/[midPoint Repository], or can represent
objects on the xref:/midpoint/reference/v1/resources/[Resources].
...
These are used for midPoint internal data, but they can also represent remote objects stored on the xref:/midpoint/reference/v1/resources/[Resources], e.g. accounts and groups.
Prism objects are stored in the xref:/midpoint/reference/v1/repository/[Repository] to preserve long-term midPoint state.
...
The resource objects are represented as xref:/midpoint/reference/v1/resources/shadow/[Shadow objects] inside midPoint.
...
For instance, when a xref:/midpoint/reference/v1/resources/connid/[ConnId Connector] is used, the Query is translated for the ConnId framework.
...
xref:/midpoint/reference/v1/concepts/query/axiom-query-language[Axiom query language] or XML.
...
This is the case of xref:/midpoint/reference/v1/security/audit/[audit records].
...
* Filters can be also written in xref:/midpoint/reference/v1/concepts/query/axiom-query-language[Axiom query language]
...
hard-core customizations (e.g. with xref:/midpoint/reference/v1/deployment/maven-overlay-project.adoc[midPoint Overlay] mechanism)
but also for xref:/midpoint/reference/v1/expressions/expressions/script/[script expressions] written typically in Groovy language.
...
* Optionally, a xref:/midpoint/reference/v1/concepts/matching-rules/[matching rule].
...
[NOTE]
Full-text search must be enabled in the system, see xref:/midpoint/reference/v1/repository/full-text-search.adoc[this document] for more information.
...
The following notes are based on the xref:/midpoint/reference/v1/repository/native-postgresql/[Native PostgreSQL repository] implementation.
...
Check also xref:/midpoint/reference/v1/schema/custom-schema-extension/#data-types-supported[supported data types for extensions]
...
[IMPORTANT]
This filter is supported only for the xref:/midpoint/reference/v1/repository/native-postgresql/[Native repository].
...
[IMPORTANT]
====
* Reference query is only supported with the xref:/midpoint/reference/v1/repository/native-postgresql/[Native repository].
...
The new xref:/midpoint/reference/v1/repository/native-postgresql/[Native repository] does not suffer
...
// TODO mention extensions, xref:/midpoint/reference/v1/schema/custom-schema-extension/
