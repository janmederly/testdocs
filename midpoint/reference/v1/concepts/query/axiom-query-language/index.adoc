...
Names of the properties, attributes and all the data items are directly taken from midPoint xref:/midpoint/reference/v1/schema/[data model].
...
The item on which data are filtered is specified by xref:/midpoint/reference/v1/concepts/item-path/[item path].
