...
If you see the exception above, please use the guide to xref:/midpoint/reference/v1/legacy/eclipse-plugin/certificate-problems/[Solving Certificate Problems].
...
The result is then in the file _com.evolveum.midpoint.eclipse.update/target/com.evolveum.midpoint.eclipse.update-x.x.x-SNAPSHOT.zip_. This is the file to be used as archive when adding new software sites (see xref:/midpoint/reference/v1/legacy/eclipse-plugin/certificate-problems/[here]).
