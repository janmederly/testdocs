...
E.g. views can be adjusted and overridden in xref:/midpoint/reference/v1/admin-gui/admin-gui-config/[admin GUI configuration].
...
E.g. It would be nice to use just "employees" as a criterion in xref:/midpoint/reference/v1/security/authorization/[authorizations] instead of copying the search filter everywhere.
...
Views are defined in xref:/midpoint/reference/v1/admin-gui/admin-gui-config/[admin GUI configuration] data structures:
...
And so on.
Placing definition of view in xref:/midpoint/reference/v1/admin-gui/admin-gui-config/[admin GUI configuration] makes it easy to specify all those details in the appropriate roles.
Therefore, all the roles will have they own customized presentation of the data.


=== Views And Implicit Collections

Some midPoint objects can be considered collections of their own, especially xref:/midpoint/reference/v1/schema/archetypes/[archetypes]. We want to make things simple and elegant whenever possible.
...
* Only partially supported for xref:/midpoint/reference/v1/admin-gui/dashboards/[dashboards] (and even that is experimental).
