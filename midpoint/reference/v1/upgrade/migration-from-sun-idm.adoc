...
Use xref:/midpoint/reference/v1/expressions/mappings/outbound-mapping/[outbound mapping] to set the value of account identifier instead of identity template.
...
MidPoint has an elegant mechanism of xref:/midpoint/reference/v1/expressions/mappings/outbound-mapping/[outbound mappings] that can be used to set resource-global attribute values.
The ability of a login role to "hold" the account in a disabled state can be done in a much easier way by using xref:/midpoint/reference/v1/resources/resource-configuration/schema-handling/activation/[activation existence mapping].