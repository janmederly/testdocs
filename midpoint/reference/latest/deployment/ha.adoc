...
. For task load balancing, the link:http://www.quartz-scheduler.org[Quartz scheduler] is used (see xref:/midpoint/reference/latest/tasks/task-manager/[architectural description]).
...
For practical information on forming a cluster, see the xref:/midpoint/reference/latest/deployment/clustering-ha/[administration guide].
