...
xref:/midpoint/reference/latest/tasks/task-manager/[Clustering / high availability] can be achieved by setting up several midPoint nodes working against common midPoint repository.
...
Alternatively, you can use the xref:/midpoint/reference/latest/deployment/midpoint-home-directory/overriding-config-xml-parameters.adoc[command line options] `-Dkey=value` to set these parameters (*Command-line parameter* in the table below).
...
(See xref:/midpoint/reference/latest/repository/configuration/[Repository Configuration]).
