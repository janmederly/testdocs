...
:page-alias: { "parent" : "/midpoint/reference/latest/repository/", "display-order" : 800 }
...
New features usually require the extension of midPoint xref:/midpoint/reference/latest/schema/[data model] to use them.
The changes of data model usually require extension of the schema of the database that stores midPoint objects.
This is the usual routine for most midPoint xref:/midpoint/reference/latest/upgrade/upgradeability/[upgrades].
...
If *Implementation name* says *Native*, you are using the new xref:/midpoint/reference/latest/repository/native-postgresql/[Native repository] recommended from version 4.4 on.
Old Generic repo says *SQL* instead.

* If in your `config.xml` the content of `repository/type` element is `native` (or `sqale`, or `scale`, casing does not matter), you are using the xref:/midpoint/reference/latest/repository/native-postgresql/[Native repository].
...
you are using the new xref:/midpoint/reference/latest/repository/native-postgresql/[Native repository].
...
This section describes how to upgrade the xref:/midpoint/reference/latest/repository/native-postgresql/[Native repository].
There is no need to "upgrade" to 4.4 - but we encourage you to xref:/midpoint/reference/latest/repository/native-postgresql/migration/[migrate to the new repository].
...
[IMPORTANT]
====
If you created the schema objects as non-superuser as described xref:/midpoint/reference/latest/repository/native-postgresql/usage/#installing-as-non-superuser[here],
...
This section describes how to upgrade the xref:/midpoint/reference/latest/repository/generic/[Generic repository].
...
See xref:/midpoint/reference/latest/repository/generic/database-schema-versioning/[Database schema versioning] for more information.
====

== See Also

* xref:/midpoint/reference/latest/upgrade/upgradeability/[Upgradeability]

* xref:/midpoint/reference/latest/upgrade/upgrade-guide/[Upgrade HOWTO]

* xref:/midpoint/reference/latest/repository/generic/database-schema-versioning/[Database schema versioning] (Generic repository)
