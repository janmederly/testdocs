...
The plan is that collections can be used in the entire midPoint system, e.g. collections can be used in xref:/midpoint/reference/latest/security/authorization/[authorizations], xref:/midpoint/reference/latest/roles-policies/policy-rules/[policy rules] may be applied to them and so on.
Some midPoint objects can act as an implicit collections, especially xref:/midpoint/reference/latest/schema/archetypes/[archetypes] and xref:/midpoint/reference/latest/org/organizational-structure/[organizational units]. Object collections are designed to be solid and stable.
...
E.g. views can be adjusted and overridden in xref:/midpoint/reference/latest/admin-gui/admin-gui-config/[admin GUI configuration].
...
Views are defined in xref:/midpoint/reference/latest/admin-gui/admin-gui-config/[admin GUI configuration] data structures:
...
And so on.
Placing definition of view in xref:/midpoint/reference/latest/admin-gui/admin-gui-config/[admin GUI configuration] makes it easy to specify all those details in the appropriate roles.
Therefore, all the roles will have they own customized presentation of the data.


=== Views And Implicit Collections

Some midPoint objects can be considered collections of their own, especially xref:/midpoint/reference/latest/schema/archetypes/[archetypes]. We want to make things simple and elegant whenever possible.
...
* Only partially supported for xref:/midpoint/reference/latest/admin-gui/dashboards/[dashboards] (and even that is experimental).
