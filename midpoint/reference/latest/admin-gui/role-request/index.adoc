...
:page-alias: [ { "parent" : "/midpoint/features/current/" }, { "parent" : "/midpoint/reference/latest/cases/" } ]
...
The request is then routed through an xref:/midpoint/reference/latest/cases/approval/[approval process]. If the request is approved, then the requested roles are assigned.
...
Therefore midPoint has a simplified view of xref:/midpoint/reference/latest/admin-gui/role-catalog/[role catalog] that is suitable for end users.
...
First, the shopping cart content is checked for any potential conflicts and policy violations, such as xref:/midpoint/reference/latest/roles-policies/segregation-of-duties/[segregation of duties] violations.
xref:/midpoint/reference/latest/roles-policies/policy-rules/[Policy rules] that apply to the roles are evaluated and enforced and the request is routed through an xref:/midpoint/reference/latest/cases/approval/[approval process] as specified by the policy.
...
For example xref:/midpoint/reference/latest/admin-gui/role-catalog/[role catalog] set up and xref:/midpoint/reference/latest/security/authorization/[authorizations] are automatically precessed, therefore only those roles that are actually requestable by the user are displayed.
However there are still few things that may need to be customized.
The customization of this part of the user interface is described in xref:/midpoint/reference/latest/admin-gui/role-request/configuration/[Role Request and Shopping Cart Configuration] page.


== See Also

* xref:/midpoint/reference/latest/admin-gui/role-catalog/[Role Catalog]

* xref:/midpoint/reference/latest/admin-gui/role-request/configuration/[Role Request and Shopping Cart Configuration]