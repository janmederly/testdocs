...
MidPoint is identity management and governance system, therefore it is quite natural that it implements an xref:/midpoint/reference/latest/cases/approval/[approval] mechanism.
...
There are xref:/midpoint/reference/latest/roles-policies/policy-rules/[policy rules] that specify individual aspects of an approvals.
...
* Assignment of controlling roles must comply with xref:/midpoint/reference/latest/roles-policies/segregation-of-duties/[segregation of duties] (SoD) policy.
