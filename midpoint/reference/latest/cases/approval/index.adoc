...
Approval is perhaps the most frequently used type of xref:/midpoint/reference/latest/cases/workflow-3/[workflow] in the IDM solutions.
...
When midPoint encounters an operation like this it will consult the policies (xref:/midpoint/reference/latest/roles-policies/policy-rules/[Policy Rules]) and decide whether the operation needs to be driven through an approval process.
...
Therefore the only thing that is usually needed to set up an approval mechanism is the definition of the approvers and the xref:/midpoint/reference/latest/roles-policies/policy-rules/[policy rules]. MidPoint will take care of the rest.
...
Assignment approvals decide whether a user can have some kind of xref:/midpoint/reference/latest/roles-policies/assignment/[assignment]. This usually means approval of a role assignment.
...
* When that operation is evaluated, xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[internal algorithms] in midPoint will analyze all the requested roles.
MidPoint will evaluate xref:/midpoint/reference/latest/roles-policies/rbac/[role hierarchies], xref:/midpoint/reference/latest/roles-policies/metaroles/gensync/[metaroles], gather all the applicable xref:/midpoint/reference/latest/roles-policies/policy-rules/[policy rules] and apply default policies.
This determines how the role assignment needs to be approved - and by whom.
This also determines whether the role can be assigned at all (e.g. whether it does not violate the xref:/midpoint/reference/latest/roles-policies/segregation-of-duties/[SoD constraints]).
...
This default can be changed by properly setting up the xref:/midpoint/reference/latest/roles-policies/policy-rules/[policy rules].

Default role approvers are defined by using the xref:/midpoint/reference/latest/roles-policies/assignment/[assignment mechanism]. Role approvers have an assignment pointing to the role that are are approving.
This assignment has xref:/midpoint/reference/latest/concepts/relation/[Relation] set to _approver_. The default midPoint policy will look for these assignments.

[TIP]
.Approval in midPoint 3.4.1 and earlier
====
MidPoint 3.4.1 and earlier haven't had the mechanism of xref:/midpoint/reference/latest/roles-policies/policy-rules/[policy rules] and therefore it used slightly different mechanism to define role approvers.
...
* When that operation is evaluated, xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[internal algorithms] in midPoint will analyze xref:/midpoint/reference/latest/roles-policies/policy-rules/[policy rules] that are applicable to the new role.
...
See xref:/midpoint/reference/latest/roles-policies/role-lifecycle/[Role Lifecycle] page for more details about specific use of lifecycle approval to control role lifecycle.
...
* xref:/midpoint/reference/latest/cases/workflow-3/[Workflows (midPoint 3.x)]
...
* xref:/midpoint/reference/latest/roles-policies/role-lifecycle/[Role Lifecycle]
