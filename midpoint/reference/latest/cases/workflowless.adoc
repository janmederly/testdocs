...
Midpoint is using xref:/midpoint/reference/latest/roles-policies/policy-rules/[policy rules] to drive approvals.
...
This is what we call xref:/midpoint/reference/latest/cases/approval/policy-based-approvals/[policy-based approvals].
...
Even though we have xref:/midpoint/reference/latest/cases/approval/policy-based-approvals/[policy-based approvals], there is still a need for a _processes._ Maybe there is an enrollment process for a new employee.
