...
It usually contains xref:/midpoint/reference/latest/expressions/[mappings] that either compute user properties or assign accounts and roles to the user (usually with a condition).
...
A user template may be applied globally by including the following snippet in xref:/midpoint/reference/latest/concepts/system-configuration-object/[system configuration]*just after the "logging" element*:
...
MidPoint is using xref:/midpoint/reference/latest/concepts/relativity/[relative changes] therefore it is not always required to read all the accounts to process a change.
...
* Use xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound mappings] instead.
...
* Use combination of xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound mappings], extended user properties and object template.
