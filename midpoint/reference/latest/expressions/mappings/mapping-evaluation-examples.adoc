...
Resource attribute can be set by several means: manually specified in midPoint user interface, produced by a mapping in a role or in xref:/midpoint/reference/latest/resources/resource-configuration/schema-handling/[resource schema handling]. MidPoint is designed to automatically merge all the values produced by mappings.
