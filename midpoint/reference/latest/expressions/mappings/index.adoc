...
It is used for example in xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mapping], xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound mapping], xref:/midpoint/reference/latest/roles-policies/assignment/[assignments], xref:/midpoint/reference/latest/roles-policies/rbac/[roles] and so on.
...
There is a broad variety of possibilities that are described in the xref:/midpoint/reference/latest/expressions/expressions/[Expression] page.
...
The most important part of mapping source definitions is specification of the xref:/midpoint/reference/latest/concepts/item-path/[item path].
The path refers to an item in an object, which is part of xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[clockwork/project] computation, which is usually a _focus_ or a _projection_.
...
Mapping sources are specified using xref:/midpoint/reference/latest/concepts/item-path/[item path], which may be quite long and complex.
...
The domain is very useful especially for automatic xref:/midpoint/reference/latest/roles-policies/assignment/[assignment] expressions in xref:/midpoint/reference/latest/expressions/object-template/[object template].
...
The xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[projector] can use this information to determine what values to remove when the mapping is authoritative.
...
Mappings are used heavily during midPoint computation (in xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[projector]).
...
Expressions are documented in a dedicated xref:/midpoint/reference/latest/expressions/expressions/[Expression] page.
...
Channel constraint limits application of a mapping to a specific xref:/midpoint/reference/latest/concepts/channel/[channel].
...
This constraint is usually used in xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound mappings] to limit them to the import channel and therefore use them only for initial import.
...
Following example provides xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound] mapping that is using xref:/midpoint/reference/latest/expressions/expressions/script/[script expression] written in Groovy language.
...
See also xref:/midpoint/reference/latest/expressions/mappings/mapping-evaluation-examples/[Mapping Evaluation Examples] page.
...
NOTE: The description above mostly applies to xref:/midpoint/reference/latest/expressions/expressions/script/[script expressions] that have to process input values one-by-one.
...
Evaluation of each value individually supports midPoint's xref:/midpoint/reference/latest/concepts/relativity/[relativity principle].
...
However, we are not that sure about the values that are present in target resource when we are about to apply the change (which may be several days later, due to xref:/midpoint/reference/latest/cases/approval/[approval process]).
...
Again, the description above mostly applies to xref:/midpoint/reference/latest/expressions/expressions/script/[script expressions].
...
The short answer is: because of xref:/midpoint/reference/latest/concepts/relativity/[relativity].
...
E.g. if this is an xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mapping] to a tolerant resource attribute.
...
For example, this approach allows easy automatic assignment of roles in xref:/midpoint/reference/latest/expressions/object-template/[object template].
