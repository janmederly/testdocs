...
There are situations, when xref:/midpoint/reference/latest/resources/shadow/[shadow objects] do not entirely reflect the reality.
Such situations are to be expected, as those "shadows" are actually copies of the data that originate from other systems.
The shadow objects also contain xref:/midpoint/reference/latest/resources/manual/configuration/[pending operation data] that naturally change in time.
...
Shadow refresh will try to finish pending operations, e.g. xref:/midpoint/reference/latest/synchronization/consistency/[re-try failed operations]. Refresh will update the status of xref:/midpoint/reference/latest/resources/manual/[manual operations]. And so on.
...
====
Refresh is not the same thing as xref:/midpoint/reference/latest/synchronization/introduction/[reconciliation]. Reconciliation is quite heavy-weight, trying to get the object from the resources, checking mappings and policies and so on.
...
Shadow refresh happens during normal operation of midPoint, e.g. it will happen during xref:/midpoint/reference/latest/synchronization/introduction/[reconciliation process]. However, sometimes it is preferable to execute just the refresh operation in a controlled manner.
