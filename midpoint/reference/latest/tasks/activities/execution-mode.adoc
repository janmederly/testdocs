...
Used for example to check the thresholds before the full (real) execution or to xref:/midpoint/reference/latest/simulation/[simulate] the execution (since 4.7).
| See note below.
...
* The `preview` execution mode is supported for the activities described xref:/midpoint/reference/latest/simulation/#_activities_supported[here].
====
