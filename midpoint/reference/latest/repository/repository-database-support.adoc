...
MidPoint stores all the data there: users, roles, xref:/midpoint/reference/latest/resources/shadow/[shadow objects] representing accounts, configuration, everything.
...
.. Import data from the XML export (e.g. using xref:/midpoint/reference/latest/deployment/ninja/[ninja]).
