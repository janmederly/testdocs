...
See xref:/midpoint/reference/latest/security/audit/[Auditing] for more general information about auditing
...
See xref:/midpoint/reference/latest/security/audit/#audit-record-structure[Audit record structure]
...
xref:/midpoint/reference/latest/upgrade/database-schema-upgrade/#upgrading-native-postgresql-repository[database schema upgrade].
...
The task consults the xref:/midpoint/reference/latest/deployment/removing-obsolete-information/[Cleanup policy]
...
[NOTE]
Audit migration is implemented in xref:/midpoint/reference/latest/deployment/ninja[Ninja] since midPoint 4.4.1.
...
This is part of the xref:/midpoint/reference/latest/repository/native-postgresql/migration/[Migration to Native PostgreSQL Repository],
...
Audit migration is performed using the xref:/midpoint/reference/latest/deployment/ninja[Ninja] tool.
...
* xref:/midpoint/reference/latest/deployment/midpoint-home-directory/[MidPoint home directory] with `config.xml` is available.
...
[NOTE]
If you use unbundled JDBC driver, check xref:/midpoint/reference/latest/deployment/ninja[Ninja documentation] how to specify the JDBC JAR.
...
. Adding the configuration for each custom column to `config.xml` file in the xref:/midpoint/reference/latest/deployment/midpoint-home-directory/[midPoint home directory].

. Adding the configuration how the audit event record property is created to the xref:/midpoint/reference/latest/concepts/system-configuration-object/[system configuration object].
...
.3. Setting up the custom property initialization - modify the xref:/midpoint/reference/latest/concepts/system-configuration-object/[system configuration object]:
[source,xml]
----
<systemConfiguration>
    ...
...
* xref:/midpoint/reference/latest/deployment/clustering-ha/[Clustering / high availability setup]
* xref:/midpoint/reference/latest/repository/native-postgresql/migration/[Migration to Native PostgreSQL Repository]
* xref:/midpoint/reference/latest/tasks/task-manager/configuration/[Task Manager Configuration]
