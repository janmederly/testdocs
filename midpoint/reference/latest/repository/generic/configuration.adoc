...
If xref:/midpoint/reference/latest/security/audit/#separate-repository-configuration-for-audit[separate audit repository] is used, this option must be used there.
...
a| (Since 4.2) If true, midPoint tries to alter tables with custom columns (currently xref:/midpoint/reference/latest/security/audit/[only in audit]) if the column is missing.
...
Anything that is externalized into columns and related tables (like extensions, references, etc.) is effectively searchable using xref:/midpoint/reference/latest/concepts/query/[midPoint Query].
