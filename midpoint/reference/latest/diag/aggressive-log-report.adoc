...
If the xref:/midpoint/reference/latest/diag/troubleshooting/usual-troubleshooting-steps/[usual troubleshooting steps] do not work, there is a time for a brutal solution.
...
The explanation of logging levels can be found on xref:/midpoint/reference/latest/diag/logging/log-levels/[Log Levels] page.
...
* What kind of resource definition was used (we prefer if you replicate the problem with one of our link:https://svn.evolveum.com/midpoint/trunk/samples/[samples], see xref:/midpoint/reference/latest/diag/creating-a-bug-report/[Creating a Bug Report].
