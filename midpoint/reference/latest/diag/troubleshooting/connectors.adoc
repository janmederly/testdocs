...
MidPoint is using the xref:/midpoint/reference/latest/resources/connid/[ConnId] connector framework.
...
See xref:/midpoint/reference/latest/diag/troubleshooting/mappings/[Troubleshooting Mappings] page for a guide how to diagnose these problems.
...
But before going to xref:/midpoint/reference/latest/diag/creating-a-bug-report/[report a bug] please spend some time diagnosing the issue.
...
. If everything fails then you can xref:/midpoint/reference/latest/diag/creating-a-bug-report/[report the issue]. However at this point it is almost sure that the issue will be very tricky.
