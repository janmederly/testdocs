...
:page-alias: { "parent" : "/midpoint/reference/latest/expressions/mappings/", "title" : "Troubleshooting", "display-order" : 800 }
:page-tag: guide
:page-toc: top
...
This page describes the most efficient steps to find out what is wrong with your xref:/midpoint/reference/latest/expressions/mappings/[mapping] setup.
...
MidPoint can be set to xref:/midpoint/reference/latest/diag/logging/[a very fine logging]. But this will produce an ocean of logging entries and it is extremely difficult to find anything meaningful in such an information overload.
...
xref:/midpoint/architecture/[MidPoint architecture] is designed in such a way that all important requests pass through the xref:/midpoint/architecture/archive/subsystems/model/[IDM Model Subsystem]. This makes sure that all requests are processed consistently, that xref:/midpoint/reference/latest/security/authorization/[authorization] is properly applied, that the request is xref:/midpoint/reference/latest/security/audit/[audited], etc.

Internal implementation of the xref:/midpoint/architecture/archive/subsystems/model/[IDM Model Subsystem] heavily relies on xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[Clockwork and Projector] components.
...
There is an InboundProcessor for xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound mappings], OutboundProcessor for xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mappings], FocusProcessor that deals with xref:/midpoint/reference/latest/expressions/object-template/[object template] and so on.
Each of the processors are invoking the same code to process xref:/midpoint/reference/latest/expressions/mappings/[mappings]. The mapping code in turn invokes xref:/midpoint/reference/latest/expressions/expressions/[expression] code to run scripts, value generators, etc.
...
Provisioning internally invokes the xref:/midpoint/reference/latest/resources/connid/[ConnId connector framework]. ConnId dispatches the invocation to a proper connector code, which executes the operation on the resource.
...
We call this a wave.
This can be repeated several times, e.g. if xref:/midpoint/reference/latest/resources/provisioning-dependencies/[provisioning dependencies] are used.
...
E.g. xref:/midpoint/reference/latest/cases/workflow-3/[workflows] and xref:/midpoint/reference/latest/misc/notifications/[notifications] are implemented as hooks.
...
Clockwork has a very convenient xref:/midpoint/reference/latest/diag/logging/useful-loggers/[logging] output:
...
* The process consisted of recomputing one xref:/midpoint/reference/latest/schema/focus-and-projections/[projection] (account) with identifier uid with value jack.
...
The most important data structure for any deeper troubleshooting of clockwork and project is xref:/midpoint/reference/latest/concepts/clockwork/model-context/[Model Context] (also called Lens Context in the implementation).
This data structure contains complete state of the computation.
Projector, clockwork and all other related components record results of the computation into the xref:/midpoint/reference/latest/concepts/clockwork/model-context/[Model Context]. Even intermediary results are sometimes recorded in the model context, therefore it is a very valuable source of troubleshooting information.
...
The xref:/midpoint/reference/latest/concepts/clockwork/model-context/[Model Context] page provides the details how is the model context structured and how to understand the information from the model context dumps.
...
There are many ways how the input deltas can influence the output: xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound mappings], xref:/midpoint/reference/latest/expressions/object-template/[object template], xref:/midpoint/reference/latest/roles-policies/rbac/[roles], xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mappings], etc.
...
* Focus Processor takes care of applying changes to the xref:/midpoint/reference/latest/schema/focus-and-projections/[focus] (usually xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/usertype/[UserType]). It takes care of xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[Inbound Mapping] and xref:/midpoint/reference/latest/expressions/object-template/[Object Template] processing.
...
xref:/midpoint/reference/latest/roles-policies/rbac/[Roles and Orgs (RBAC)] is also pre-computed in this stage.
Although we only care whether a specific xref:/midpoint/reference/latest/schema/focus-and-projections/[projection] is legal or not.
We do not process the projection attributes yet.

* Activation Processor determines which xref:/midpoint/reference/latest/schema/focus-and-projections/[projections] should exist and what is their activation state (enabled/disabled, time constraints, etc.)

* Then each xref:/midpoint/reference/latest/schema/focus-and-projections/[projection] is processed individually:

** Projection Values Processor will evaluate attribute values specified in xref:/midpoint/reference/latest/roles-policies/rbac/[roles and Orgs (RBAC)] and xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mappings]. All the mappings are evaluated and the values are consolidated into deltas.
...
Computation in waves is needed e.g. if there are xref:/midpoint/reference/latest/resources/provisioning-dependencies/[provisioning dependencies] and the resources must be processed in a strict order.
However the computation is usually done at least in "one and a half wave".
The last half-wave is added to make sure that the results of the provisioning operations are correctly reflected to the xref:/midpoint/reference/latest/schema/focus-and-projections/[focus] by the means of xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound mappings].
...
The best overview of the projection operation is given the the xref:/midpoint/reference/latest/concepts/clockwork/model-context/[Model Context] dumps (see above).
...
. Enable xref:/midpoint/reference/latest/concepts/clockwork/model-context/[Model Context] dumps from projector (com.evolveum.midpoint.model.impl.lens.projector.Projector: TRACE)
...
MidPoint is based on xref:/midpoint/reference/latest/concepts/relativity/[the principle of relative changes]. Therefore midPoint will deal with operations as if they add or remove something.
...
. Input deltas are recorded in xref:/midpoint/reference/latest/concepts/clockwork/model-context/[Model Context] as primary deltas or synchronization deltas

. xref:/midpoint/reference/latest/expressions/mappings/[Mappings] are evaluated.
...
Similar logic also applies to processing xref:/midpoint/reference/latest/roles-policies/assignment/[assignments]. There are also triples that describe which assignments will be added or removed.
...
If you find out the problem is somewhere inside a xref:/midpoint/reference/latest/expressions/expressions/script/[Script Expression] code then there is way how to troubleshoot that.
MidPoint has xref:/midpoint/reference/latest/expressions/expressions/script/functions/[script expression functions] that can be used to log messages from the scripting code.
It works like this:
...
See xref:/midpoint/reference/latest/expressions/expressions/script/functions/[Script Expression Functions] page for more details.
...
This kind of a "terminal station" where the xref:/midpoint/reference/latest/concepts/clockwork/model-context/[model context] arrives to be executed.
...
The ChangeExecutor will execute the changes on xref:/midpoint/reference/latest/schema/focus-and-projections/[focal objects] directly in midPoint repository.
...
E.g. there may be xref:/midpoint/reference/latest/resources/resource-configuration/simulated-disable/[simulated activation] or xref:/midpoint/reference/latest/resources/entitlements/[entitlement associations] to process.
...
. xref:/midpoint/reference/latest/resources/entitlements/[Entitlement associations] are processed again.
...
Mail article: xref:/midpoint/reference/latest/diag/troubleshooting/connectors/[Troubleshooting Connectors]
...
See xref:/midpoint/reference/latest/diag/troubleshooting/connectors/[Troubleshooting Connectors] for more details.
...
* Enable xref:/midpoint/reference/latest/security/audit/[Auditing] to a log file, including the auditing details.
...
* xref:/midpoint/reference/latest/diag/troubleshooting/connectors/[Troubleshooting Connectors]
