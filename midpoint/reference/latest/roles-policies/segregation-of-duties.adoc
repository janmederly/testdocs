...
In midPoint 3.5 and later the exclusion is defined by using the xref:/midpoint/reference/latest/roles-policies/policy-rules/[policy rules] mechanism:
...
xref:/midpoint/reference/latest/roles-policies/policy-rules/[Policy Rules] have two parts important for role exclusion configurations: _constraints_ and _action_. _Constraints_ part defines which roles are exclude which roles.
...
However this configuration can easily be created an maintained by using xref:/midpoint/reference/latest/roles-policies/rbac/[role hierarchies] and xref:/midpoint/reference/latest/roles-policies/metaroles/gensync/[metaroles].
...
And while currently there are no specific plans to remove it we recommend to migrate to configuration based on xref:/midpoint/reference/latest/roles-policies/policy-rules/[Policy Rules] at the earliest convenient moment.
The policy rules are much more flexible mechanism.

== History

Basic role exclusion with strict enforcement is available in midPoint since version 2.x. The definition based on xref:/midpoint/reference/latest/roles-policies/policy-rules/[Policy Rules] is available since midPoint 3.5. However this is still limited to strict enforcement.
