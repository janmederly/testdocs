...
:page-alias: [ { "parent" : "/midpoint/features/current/" }, { "parent" : "/midpoint/reference/latest/admin-gui/" } ]
:page-upkeep-status: yellow

== Introduction

MidPoint is very flexible system, especially when it comes to xref:/midpoint/reference/latest/cases/approval/[approval] and other xref:/midpoint/reference/latest/roles-policies/policy-rules/[policies]. But the flexibility comes at a cost.
...
The basic idea is to express the policies in a form of xref:/midpoint/reference/latest/roles-policies/metaroles/policy/[metaroles]. For example the policy that drives role assignment through an approval will be expressed in "approval by manager" metarole.
This metarole will contain all the xref:/midpoint/reference/latest/roles-policies/policy-rules/[policy rules] necessary to express the approval policy.
...
And that is what we call xref:/midpoint/reference/latest/roles-policies/metaroles/gensync/[metarole]. The situation looks like this:
...
But those rules are specified inside xref:/midpoint/reference/latest/roles-policies/assignment/assignment-vs-inducement/[inducement]. Therefore the policy rules do not apply to the role which contains them, i.e. the rules do not apply to the "Approval by manager metarole".
...
The xref:/midpoint/reference/latest/roles-policies/metaroles/policy/[Using Metaroles for Policy Configuration] page describes the basic principle and also provides some examples.
...
The mechanism is xref:/midpoint/reference/latest/org/organizational-structure/[organizational structure]. This has additional benefit that the policy categories can be easily managed by using concepts of delegated administration which is common for organizational structures.
...
Please see xref:/midpoint/reference/latest/roles-policies/applicable-policies/configuration/[Applicable Policy Configuration] page for configuration details and examples.
...
Global policy rules can be used to apply the rules to all selected objects based on a xref:/midpoint/reference/latest/concepts/query/[filter]. Therefore the policies can be applied in a systemic way.
