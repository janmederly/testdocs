...
The xref:/midpoint/reference/latest/expressions/object-template/[object template] is the right place to implement such automatic assignment.
...
This is easy to implement using an object template xref:/midpoint/reference/latest/expressions/mappings/[mapping] with a static (literal) value:
...
When the condition becomes `false` the role is unassgined (remebmer, midPoint is xref:/midpoint/reference/latest/concepts/relativity/[relativistic] and that also applies to conditions).
...
MidPoint has special `assignmentTargetSearch`xref:/midpoint/reference/latest/expressions/expressions/[expression evaluator] designed especially for this purpose.
...
E.g. the target (role or org) can be dynamically created on demand (see xref:/midpoint/reference/latest/samples/story-tests/orgsync/[OrgSync Story Test]). Mapping domain and range can be used for more complex cases.
And so on.
