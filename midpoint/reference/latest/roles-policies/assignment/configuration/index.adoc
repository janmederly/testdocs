...
Assignment is a generic concept of associating user (or other xref:/midpoint/reference/latest/schema/focus-and-projections/[focal object]) with the things that he should have or belong to.
...
The following figure illustrates the concept and the xref:/midpoint/reference/latest/roles-policies/assignment/assigning-vs-linking/[Assigning vs Linking] page describes it in more details.
...
Any xref:/midpoint/reference/latest/schema/focus-and-projections/[focal] object can have assignments.
This mechanism is used to implement xref:/midpoint/reference/latest/synchronization/generic-synchronization/[generic synchronization].
...
See xref:/midpoint/reference/latest/roles-policies/assignment/assignment-vs-inducement/[Assignment vs Inducement] page for a detailed explanation.
...
E.g. in xref:/midpoint/reference/latest/synchronization/generic-synchronization/[generic synchronization] cases an assignment may assign an xref:/midpoint/reference/latest/resources/entitlements/[entitlement] (e.g. a group) to midPoint xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/roletype/[role.] The `kind` and `intent` elements in the construction part define what resource object is actually assigned.
See xref:/midpoint/reference/latest/resources/shadow/kind-intent-objectclass/['Kind, Intent and ObjectClass'] page for more details.
...
The later case is a very devious (ab)use of xref:/midpoint/reference/latest/synchronization/generic-synchronization/[generic synchronization].
...
See the xref:/midpoint/reference/latest/resources/entitlements/[Entitlements] page for more details.
...
An observant reader had surely noticed similarity with xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mappings].
The same xref:/midpoint/reference/latest/expressions/mappings/[mapping] principle is used in both outbound mappings and assignments.
...
Most importantly the values are merged with the results of xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mappings].
...
Care should be taken even if this ability is used in a form of xref:/midpoint/reference/latest/roles-policies/rbac/[role inducements].
...
MidPoint has a clean and elegant mechanism for specifying resource-global policies: xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mappings].

*Rule of the thumb:* xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[Outbound mappings] should be used to set attribute values if possible.
If not then xref:/midpoint/reference/latest/roles-policies/rbac/[role inducements] should be used.
...
xref:/midpoint/reference/latest/resources/entitlements/[Entitlements] are resource objects that represent groups, privileges, resource roles or similar objects that can be associated to accounts.
Similarly to attribute values, assignments can also specify entitlement associations.
The simplest way how to specify entitlement association in an assignment is to directly specify entitlement's xref:/midpoint/reference/latest/resources/shadow/[shadow]xref:/midpoint/devel/prism/concepts/object-identifier/[OID:]
...
====
Please keep in mind that xref:/midpoint/reference/latest/resources/entitlements/[entitlements] are resource objects such as groups, privileges or ACLs.
As all resource objects the entitlements are represented by xref:/midpoint/reference/latest/resources/shadow/[shadow objects] in midPoint.
...
There is a special-purpose `associationTargetSearch`xref:/midpoint/reference/latest/expressions/expressions/[expression evaluator] that allows significant flexibility.
This type of expression is using a xref:/midpoint/reference/latest/concepts/query/[search query] to dynamically locate the entitlement shadow.
...
But when it is combined with a xref:/midpoint/reference/latest/roles-policies/metaroles/gensync/[metarole] mechanism then it becomes an extremely powerful tool.
...
See xref:/midpoint/reference/latest/roles-policies/assignment/using-assignment-path-in-expressions/[Using Assignment Path in Expressions] and xref:/midpoint/reference/latest/roles-policies/assignment/assignment-path-variables-example/[Assignment Path Variables Example]


|===

Use of these variables is xref:/midpoint/reference/latest/roles-policies/assignment/assignment-path-variables-example/[illustrated on the example here].
...
Things are getting a bit more complex when xref:/midpoint/reference/latest/synchronization/generic-synchronization/[generic synchronization] feature is used.
...
As the xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/orgtype/[Org] is a xref:/midpoint/reference/latest/schema/focus-and-projections/[focal object] it can have assignments.
...
This assignment states that an xref:/midpoint/reference/latest/resources/entitlements/[entitlement] (which is a resource object) should be created.
...
Therefore for this approach to work well you need to do the same as for accounts: define entitlement in xref:/midpoint/reference/latest/resources/resource-configuration/schema-handling/[Resource Schema Handling] which usually includes some xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mappings] to set group identifiers and other attributes.
The xref:/midpoint/reference/latest/resources/entitlements/[Entitlements] page has some examples how to do this.

[TIP]
.Intent
====
xref:/midpoint/reference/latest/resources/shadow/kind-intent-objectclass/[Intent] setting is usually not used often when working with accounts.
...
And this is usually specified by using xref:/midpoint/reference/latest/resources/shadow/kind-intent-objectclass/[intent].
...
However as xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/roletype/[RoleType] is a xref:/midpoint/reference/latest/schema/focus-and-projections/[focal object] it is perfectly legal for a role to contain an *assignment*.
Such assignment applies to a role itself and it can be used e.g. to create a group as a representation of the role.
See xref:/midpoint/reference/latest/roles-policies/assignment/assignment-vs-inducement/[Assignment vs Inducement] page for more details.
Also have a look at xref:/midpoint/reference/latest/roles-policies/rbac/[Advanced Hybrid RBAC] page for an interesting discussion of meta-roles.
...
Obviously a role can be assigned to many users therefore creating an xref:/midpoint/reference/latest/roles-policies/rbac/[RBAC] structure.
...
The xref:/midpoint/reference/latest/roles-policies/rbac/[Advanced Hybrid RBAC] and xref:/midpoint/reference/latest/roles-policies/roles-and-policies-configuration/[Roles and Policies Configuration] pages provide much more details about RBAC and role definitions.
...
This kind of assignment parameters, called xref:/midpoint/reference/latest/concepts/activation/[activation], is pre-defined in midPoint.
...
See xref:/midpoint/reference/latest/roles-policies/rbac/[Advanced Hybrid RBAC] for more details.

== Enforcement Modes

Main article: xref:/midpoint/reference/latest/synchronization/projection-policy/[Projection Policy]

There are many ways how to resolve the difference between the policy (assignments) and reality.
The goal of xref:/midpoint/reference/latest/synchronization/projection-policy/[enforcement modes] is to determine account _legality_.
...
The legality of the account is then used by the xref:/midpoint/reference/latest/resources/resource-configuration/schema-handling/activation/[activation] mechanism to determine what to do with the account.
...
Assignments are often created automatically by midPoint mappings, especially in the xref:/midpoint/reference/latest/expressions/object-template/[object template].
It is perfectly possible to create assignments using a `value`xref:/midpoint/reference/latest/expressions/expressions/[expression evaluator].
However this method works well only for simple cases.
Therefore there is a specialized `assignmentTargetSearch`xref:/midpoint/reference/latest/expressions/expressions/[expression evaluator] designed for the very specific purpose of creating assignments.
It is using a xref:/midpoint/reference/latest/concepts/query/[search query] to dynamically locate assignment targets.
It can be configured using inner expressions which makes it a very powerful mechanism.
See xref:/midpoint/reference/latest/expressions/expressions/[Expression] page for more details.
...
* xref:/midpoint/reference/latest/roles-policies/assignment/[Assignment] feature description

* xref:/midpoint/reference/latest/synchronization/generic-synchronization/[Generic Synchronization]

* xref:/midpoint/reference/latest/resources/entitlements/[Entitlements]

* xref:/midpoint/reference/latest/synchronization/projection-policy/[Projection Policy]

* xref:/midpoint/reference/latest/roles-policies/assignment/assigning-vs-linking/[Assigning vs Linking]

* xref:/midpoint/reference/latest/roles-policies/assignment/assignment-vs-inducement/[Assignment vs Inducement]

* xref:/midpoint/reference/latest/resources/resource-configuration/schema-handling/activation/[Resource Schema Handling: Activation]

* xref:/midpoint/reference/latest/schema/focus-and-projections/[Focus and Projections]

* xref:/midpoint/reference/latest/expressions/expressions/[Expression], especially part about assignment expressions
