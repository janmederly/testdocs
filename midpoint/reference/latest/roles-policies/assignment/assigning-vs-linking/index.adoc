...
If an account is assigned to the user the xref:/midpoint/reference/latest/roles-policies/assignment/[assignment] structure is added to the user object.
...
Note that the mechanism of xref:/midpoint/reference/latest/roles-policies/assignment/[assignments] is actually the same concept that is used to model xref:/midpoint/reference/latest/roles-policies/rbac/[roles]. Simply speaking roles can be seen as indirect assignments.
...
For that reason there is an xref:/midpoint/reference/latest/synchronization/projection-policy/[option in the system configuration] to weaken enforcement of assignment-link consistency.
