...
It is used in xref:/midpoint/reference/latest/expressions/[mappings] and configuration.
...
It is not stored in the xref:/midpoint/reference/latest/repository/[repository (database)]
and cannot be dynamically changed when the system is running.

The schema files have to be placed in the `schema` subdirectory of xref:/midpoint/reference/latest/deployment/midpoint-home-directory/[midpoint.home directory].
...
h| How is it stored in xref:/midpoint/reference/latest/repository/native-postgresql/[Native repository]?
h| How is it stored in xref:/midpoint/reference/latest/repository/generic/[Generic repository]?
...
See xref:/midpoint/reference/latest/repository/native-postgresql/db-maintenance/#index-tuning[Index tuning] for more information.
