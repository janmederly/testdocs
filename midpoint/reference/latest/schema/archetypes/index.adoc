...
A _project_ is just a special case of xref:/midpoint/reference/latest/org/organizational-structure/[organizational unit]. As is _division_, _section_, _company_, _workgroup_ or any other concept that groups identities.
An _application_ is a special case of xref:/midpoint/reference/latest/misc/services/[service]. As is _mobile device_, _printer_, _cloud service_, _web API endpoint_ and so on.
...
.Synergy
====
Archetype functionality is a xref:/midpoint/features/synergy/[synergistic feature]. It is designed to fit together with existing midPoint features, such as xref:/midpoint/reference/latest/roles-policies/rbac/[RBAC] and xref:/midpoint/reference/latest/org/organizational-structure/[organizational structure].
====
...
That means archetypes may work as xref:/midpoint/reference/latest/roles-policies/metaroles/policy/[metaroles]. Or it may the other way around: archetypes may be assigned from ordinary roles or orgs.
...
====
MidPoint 3.x used mechanism of xref:/midpoint/reference/latest/schema/subtype/[subtype] to implement parts of the functionality provided by archetypes.
...
Archetypes are integrated with xref:/midpoint/reference/latest/admin-gui/collections-views/[views]. Archetypes can be used as an implicit collection.
...
Archetypes can also be used with xref:/midpoint/reference/latest/admin-gui/collections-views/[collections]. MidPoint support some kind of inheritance of collections.
...
This means that archetype can limit the scope of objects and xref:/midpoint/reference/latest/concepts/relation/[relations] that are used in assignments to the archetyped object.
