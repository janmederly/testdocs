...
For an introduction, please see xref:/midpoint/reference/latest/correlation/#fuzzy-searching[Fuzzy Searching] section in the overview document.
...
NOTE: The overall confidence value may be later scaled using a custom _rule weight_ when rules are composed together, as described in xref:/midpoint/reference/latest/correlation/rule-composition/[rule composition] document.
...
This feature is available only when using the xref:/midpoint/reference/latest/repository/native-postgresql/[native repository implementation].
