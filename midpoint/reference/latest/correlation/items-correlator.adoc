...
(We assume that the value of the property for accounts being correlated is determined by xref:/midpoint/projects/idmatch/implementation/correlation-time-mappings/[correlation-time evaluation of inbound mappings], as described in the xref:/midpoint/reference/latest/correlation/[overview document].)
...
For more information please see xref:/midpoint/reference/latest/correlation/fuzzy-searching/[fuzzy searching] document.
