...
Type of every xref:/midpoint/reference/latest/resources/shadow/[resource objects] that midPoint deals is defined by its xref:/midpoint/reference/latest/resources/shadow/kind-intent-objectclass/[object class]. The object class defines the basic structure of the object: whether it is account, group or organizational unit.
...
In that case simply add `auxiliaryObjectClass` configuration property to the usual object definition in xref:/midpoint/reference/latest/resources/resource-configuration/[resource configuration].
...
The mechanism that usually decides whether a user should have UNIX account or not is xref:/midpoint/reference/latest/roles-policies/rbac/[RBAC]. If the user has a role that gives a UNIX account then the account should have `posixAccount` auxiliary object class.
...
In this case the xref:/midpoint/reference/latest/resources/resource-configuration/[resource configuration] contains only the structural object class:
...
See the xref:/midpoint/reference/latest/samples/story-tests/unix/[Unix Story Test] for a complete configuration example.
====
