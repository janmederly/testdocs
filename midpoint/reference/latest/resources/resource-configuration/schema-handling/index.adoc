...
Resource Schema Handling is a definition of how the xref:/midpoint/reference/latest/resources/resource-schema/[Resource Schema] is used by midPoint.
...
The Resource Schema Handling is related to the xref:/midpoint/reference/latest/resources/resource-schema/[Resource Schema], but there is a fundamental distinction:

* xref:/midpoint/reference/latest/resources/resource-schema/[Resource Schema] specifies *capabilities of the Resource and Connector*. If does not define how the schema is used (although it may suggest it).
...
While xref:/midpoint/reference/latest/resources/resource-schema/[Resource Schema] is usually generated from the Resource by the connector, the Resource Schema Handling must be manually defined by the IDM administrator.
...
Accounts are data records representing user on the Resource, entitlements are objects that can be assigned to accounts (such as groups, roles, privileges, team membership, etc.). See xref:/midpoint/reference/latest/schema/data-model-essentials/[Basic Data Model] for the introduction to these concepts.
...
See the xref:/midpoint/reference/latest/resources/shadow/kind-intent-objectclass/['Kind, Intent and ObjectClass'] page for more details on kind and intent.


=== Object Class

xref:/midpoint/reference/latest/resources/resource-schema/[Object Class] is a type of the object how the _resource_ understands it.
...
The definition specifies what xref:/midpoint/reference/latest/resources/resource-schema/[Object Class] to use for such objects, how to handle the attributes, how to determine attribute values, etc.
...
It must refer to exiting definition in the xref:/midpoint/reference/latest/resources/resource-schema/[Resource Schema]. This is the _structural_ (primary) object class of the object.
...
** *xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[Outbound Mapping]* defines how the attribute value is created when the account is created or updated.
It defines the data flow out of midPont to the resource.

** *xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[Inbound Mapping]* defines how the attribute value is used when a change of the account is detected on the Resource.
...
** *Password* defines a password synchronization properties, it usually contains xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[Outbound Mapping] definition.
It may also contain xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[Inbound Mapping] but as most resources hash their password this has a very limited use.



* *xref:/midpoint/reference/latest/resources/resource-configuration/schema-handling/activation/[Activation]* handling defines how the activation status of the account is synchronized.
...
It may contain xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound] and xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound] mappings.
...
Perhaps the most powerful parts of the definition are xref:/midpoint/reference/latest/expressions/mappings/[mappings] that take two slightly distinct forms:
...
Such defaults are specified in the xref:/midpoint/reference/latest/resources/resource-schema/[Resource Schema] in a form of XSD annotations.
...
* xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[Inbound Mapping]

* xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[Outbound Mapping]

* xref:/midpoint/reference/latest/expressions/mappings/mapping-evaluation-examples/[Mapping Evaluation Examples]
