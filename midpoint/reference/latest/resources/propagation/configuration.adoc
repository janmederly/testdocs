...
But this may be quite troublesome for xref:/midpoint/reference/latest/resources/manual/[manual resources] and slow resources where the operations are costly.
...
See xref:/midpoint/reference/latest/resources/propagation/[Provisioning Propagation] page for full introduction to this topic.
...
All the operations will be queued in xref:/midpoint/reference/latest/resources/shadow/[shadow objects] in a form of pending deltas.
...
The task looks for any xref:/midpoint/reference/latest/resources/shadow/[shadow] with a pending operation that is older than the grouping interval specifies.
...
Current implementation of provisioning propagation was designed specifically to work with simple xref:/midpoint/reference/latest/resources/manual/[manual resources]. Therefore there are some limitations:

* Provisioning scripts are currently not supported.
The propagation task will ignore all provisioning scripts.

* Support for xref:/midpoint/reference/latest/resources/entitlements/[entitlement associations] is limited.
...
* xref:/midpoint/reference/latest/synchronization/consistency/[Consistency mechanism] support for propagated resources is currently limited.
