...
A typical example is an xref:/midpoint/reference/latest/cases/workflow-3/[approval workflow]: a user requests an operation that has to be made, but the operation (or some parts of it) may be subject of an approval.
