...
Model context is a complex data structure that describes the context of computation in xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[Clockwork and Projector] components.
It is also used in xref:/midpoint/reference/latest/concepts/clockwork/hooks/[Hooks], xref:/midpoint/reference/latest/concepts/clockwork/scripting-hooks/[Scripting Hooks], xref:/midpoint/reference/latest/cases/workflow-3/[Workflows (midPoint 3.x)] and other components that take part in the computation.

image::clockwork-projector-model-context.png[]



The model context is putting together the xref:/midpoint/reference/latest/schema/focus-and-projections/["focus" and "projections"] that belong together.
...
Model Context has three parts (see xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[Clockwork and Projector] page for explanation of terms used here):
...
There is at most one focus (see xref:/midpoint/reference/latest/schema/focus-and-projections/[Focus and Projections]).

* *Projection* part which contains information about each projection.
There may be multiple projections (see xref:/midpoint/reference/latest/schema/focus-and-projections/[Focus and Projections]).
...
This is the delta that was explicitly entered in the GUI, supplied to the web service or otherwise specified in xref:/midpoint/reference/latest/interfaces/model-java/[IDM Model Interface] invocation.
...
"Model Context" is a public name of this data structure as it is exposed by the xref:/midpoint/reference/latest/interfaces/model-java/[IDM Model Interface] (midPoint Model API).
