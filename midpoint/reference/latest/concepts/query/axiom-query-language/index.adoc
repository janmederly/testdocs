...
Names of the properties, attributes and all the data items are directly taken from midPoint xref:/midpoint/reference/latest/schema/[data model].
...
The item on which data are filtered is specified by xref:/midpoint/reference/latest/concepts/item-path/[item path].
