...
Also roles, organizational units and even individual xref:/midpoint/reference/latest/roles-policies/assignment/[assignments] can have individual activation properties.
...
Unless the activation is constrained by the xref:/midpoint/reference/latest/concepts/object-lifecycle/[object lifecycle] (see below).
...
Activation is also influenced by xref:/midpoint/reference/latest/concepts/object-lifecycle/[object lifecycle]. The object behaves normally in `active` and `deprecated` lifecycle states.
