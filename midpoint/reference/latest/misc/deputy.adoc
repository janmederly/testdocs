...
xref:/midpoint/reference/latest/cases/workflow-3/[Workflows (midPoint 3.x)] and xref:/midpoint/reference/latest/roles-policies/certification/[access certification] fully support the concept of deputy.
...
See xref:/midpoint/reference/latest/cases/approval/[Approval] page for more details.
====
...
Deputy delegation are in fact xref:/midpoint/reference/latest/roles-policies/assignment/[assignments]. that have special `deputy`xref:/midpoint/reference/latest/concepts/relation/[relation] in their target reference.
...
Normal assignment between objects that are already assigned would cause similar effect that is used by xref:/midpoint/reference/latest/roles-policies/metaroles/gensync/[metaroles]. But the delegation (deputy) assignment does not do this.
...
When one user is changed the other one is unlikely to be directly affected because only one xref:/midpoint/reference/latest/schema/focus-and-projections/[focal object] is processed in every midPoint operation.
...
See xref:/midpoint/reference/latest/roles-policies/metaroles/gensync/['Roles, Metaroles and Generic Synchronization'] page for details.
...
* xref:/midpoint/reference/latest/concepts/relation/[Relation]
