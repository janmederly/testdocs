...
xref:/midpoint/reference/latest/repository/native-postgresql/[Native repository] with midPoint 4.7 or newer.

This report demonstrates the following features:

* xref:/midpoint/reference/latest/concepts/query/#reference-query[Reference search] - we directly search for `roleMembershipRef` references.
* xref:/midpoint/reference/latest/misc/value-metadata/[Value metadata] - we work with extended assignment information stored inside value metadata in the reference values.
* xref:/midpoint/reference/latest/misc/reports/configuration/#subreports-generating-rows[Row generation] - we generate new rows depending on the number of value metadata values.
* The report also shows a few interesting
xref:/midpoint/reference/latest/expressions/expressions/script/[script expressions]
with the xref:/midpoint/reference/latest/expressions/expressions/script/groovy/[Groovy Expressions].
...
<1> The report is based on a xref:/midpoint/reference/latest/concepts/query/#reference-query[Reference query]
...
has xref:/midpoint/reference/latest/misc/reports/configuration/#subreports-generating-rows[its own row].
xref:/midpoint/reference/latest/misc/reports/report-functions/[Report function] `generateAssignmentPathRows`
...
* xref:/midpoint/reference/latest/misc/reports/configuration/[Report configuration]

* xref:/midpoint/reference/latest/concepts/query/#reference-query[Reference Query]
