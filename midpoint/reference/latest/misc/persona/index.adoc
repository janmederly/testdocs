...
The personal links are not just simple links between user objects (`UserType`). These links may be couples with xref:/midpoint/reference/latest/expressions/mappings/[mappings]. E.g. the mappings may be used to propagate the changes from the physical user to the personas.
...
MidPoint can use ordinary xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound mappings] to get the data from the HR system to the physical user object.
The persona link mappings can then be used to propagate these changes to all the linked persona users.
And then again ordinary xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mappings] can be used to propagate the changes to all affected resources.
...
It is called "xref:/midpoint/reference/latest/resources/shadow/kind-intent-objectclass/[intent]". The intent can be used to maintain several accounts for one user.
...
Please see xref:/midpoint/reference/latest/misc/persona/configuration/[Persona Configuration] page for configuration details.

== See Also

* xref:/midpoint/reference/latest/schema/data-model-essentials/[Basic Data Model]
...
* xref:/midpoint/reference/latest/misc/persona/configuration/[Persona Configuration]
