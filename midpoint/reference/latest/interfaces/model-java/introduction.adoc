...
This interface mostly work with midPoint objects as described in our xref:/midpoint/reference/latest/schema/[Data Model]. Please see the xref:/midpoint/reference/latest/schema/data-model-essentials/[Basic Data Model] page for a quick introduction to the data model.
The xref:/midpoint/reference/latest/schema/[Data Model] page expands on that and provides more details.
...
Other objects reside partially in midPoint repository and partially on remote resources (e.g. xref:/midpoint/reference/latest/resources/shadow/[Shadow Objects]). However this distinction is mostly hidden from the user of IDM Model Interface.
...
MidPoint is designed to work with xref:/midpoint/reference/latest/concepts/relativity/[relative change model.] This model is built deeply into the basic midPoint functionality.
...
Please note that most objects are using xref:/midpoint/reference/latest/concepts/polystring/[PolyString] instead of plain strings for some attributes.
The use of xref:/midpoint/reference/latest/concepts/polystring/[PolyString] provides important advantage in internationalized environment.
...
More sophisticated error handling is possible by inspecting the xref:/midpoint/architecture/concepts/operation-result/[Operation Result]. E.g. if a user is modified, the modification is reflected to several accounts using xref:/midpoint/reference/latest/expressions/mappings/[mappings], some of account modification succeed and one of the account modification fails then an exception is *not* thrown.
...
The key to search methods is xref:/midpoint/reference/latest/concepts/query/[midPoint query language]. This query language is specific to midPoint and it describes an abstract tree of search criteria.
...
The query describes objects for which the `name` property (specified as `ResourceType.F_NAME`) equals to xref:/midpoint/reference/latest/concepts/polystring/[PolyString]`"My Resource`". Once executed this will return all such xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/resourcetype/[ResourceType] objects (which obviously is at most one object as `name` property is unique across all the resources).
...
* xref:/midpoint/reference/latest/interfaces/invoking-midpoint-services/[Invoking MidPoint Services HOWTO]
