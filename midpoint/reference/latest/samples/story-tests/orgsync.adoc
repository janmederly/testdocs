...
* Create user records from the HR employee records (basic inbound xref:/midpoint/reference/latest/synchronization/introduction/[synchronization]).
...
* Replicate midPoint organizational structure to LDAP organizational structure (xref:/midpoint/reference/latest/synchronization/generic-synchronization/[generic synchronization]).

* xref:/midpoint/reference/latest/roles-policies/assignment/[Assign] users to appropriate organizational units.
...
* Use of xref:/midpoint/reference/latest/expressions/object-template/[object template] and meta-role as a configuration of organizational structure xref:/midpoint/reference/latest/synchronization/generic-synchronization/[generic synchronization] policy.
...
This approach is used to a great advantage: we can use just a handful of simple xref:/midpoint/reference/latest/expressions/mappings/[mappings] to support very complex scenarios.
...
. The account data are copied to the new user by xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound mappings] on HR resource.
...
It is using a xref:/midpoint/reference/latest/concepts/query/[query] inside xref:/midpoint/reference/latest/expressions/expressions/[assignmentTargetSearch expression] to do so.
...
Similarly to the previous case xref:/midpoint/reference/latest/expressions/expressions/[assignmentTargetSearch expression] with a query is used.
...
This meta-role contains `inducement` which specifies that a new `ou` resource object should be created as a xref:/midpoint/reference/latest/schema/focus-and-projections/[projection] for each Org.
 +


.... The projections are computed for an LDAP resource.
The projections have a form of LDAP organizationalUnit objects.
This is defined in the xref:/midpoint/reference/latest/resources/resource-configuration/schema-handling/[schema handling] part of LDAP resource definition.
The inducement specifies a xref:/midpoint/reference/latest/resources/shadow/kind-intent-objectclass/['(kind, intent)'] tuple which is used to locate a matching definition in the schemaHandling.

.... The xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mappings] are used to compute a correct DN for the new "ou" object in LDAP.
...
And as they all have the meta-role, corresponding LDAP organizationalUnit xref:/midpoint/reference/latest/schema/focus-and-projections/[projection] is created for each of them.
...
This just assigns the "Basic User" role `(role-basic.xml`) . This is a simple xref:/midpoint/reference/latest/roles-policies/rbac/[RBAC] role that assigns an LDAP account to the user.
...
The xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mappings] from the OpenDJ resource definition are used to fill in account attribute values.
...
.... The meta-role contains an inducement which specifies that a xref:/midpoint/reference/latest/schema/focus-and-projections/[projection] should be created for the role on the LDAP resource.
The projection should be a group entitlement.
This is specified by xref:/midpoint/reference/latest/resources/shadow/kind-intent-objectclass/['(kind, intent)'] tuple which points to the xref:/midpoint/reference/latest/resources/resource-configuration/schema-handling/[schema handling] part of LDAP resource definition.

.... The xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mappings] are used to compute a correct DN for the new group object in LDAP.
...
A group is created on LDAP as a xref:/midpoint/reference/latest/schema/focus-and-projections/[projection] of that role: `cn=R_canibalism,ou=groups,dc=example,dc=com`. The group is empty for now.
...
Fortunately midPoint has a smart xref:/midpoint/reference/latest/resources/entitlements/[entitlements] mechanism.
The xref:/midpoint/reference/latest/resources/resource-configuration/schema-handling/[schema handling] part of LDAP resource definition contains a definition of association between account and group which defines that the actual direction of the association is the other way around.
