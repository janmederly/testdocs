...
As long as appropriate xref:/midpoint/reference/latest/schema/custom-schema-extension/[extension schema] is created and deployed to a midPoint instance, midPoint will automatically understand the data format.
...
MidPoint is configured to use this class as a custom form in the xref:/midpoint/reference/latest/admin-gui/admin-gui-config/[admin GUI configuration] section of the operator role:
...
Actual transformation of the data is implemented in a xref:/midpoint/reference/latest/roles-policies/metaroles/gensync/[metarole]. All organizational units that contain specific transform tables are assigned to this metaroles.
...
Files used to implement this sample can be used in xref:/midpoint/reference/latest/deployment/maven-overlay-project/[example overlay project]:
...
* link:https://github.com/Evolveum/midpoint-overlay-example/blob/master/src/main/resources/initial-objects/900-role-superoperator.xml[900-role-superoperator.xml]: operator role which contains custom form definition (using xref:/midpoint/reference/latest/admin-gui/admin-gui-config/[admin GUI configuration])
...
* xref:/midpoint/reference/latest/deployment/maven-overlay-project/[Customization With Overlay Project]