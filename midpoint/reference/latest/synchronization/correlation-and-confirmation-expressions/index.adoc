...
xref:/midpoint/reference/latest/synchronization/introduction/[Synchronization] mechanism makes sure that the state of accounts corresponds to the state of the user that owns them with respect to the configured policies.
For this to work properly there needs to be a way how to set which user owns which account.
Account xref:/midpoint/reference/latest/roles-policies/assignment/assigning-vs-linking/[linking] is used as a primary mechanisms for this.
The links are created automatically when account is created as a consequence of user modification (e.g. a role is xref:/midpoint/reference/latest/roles-policies/assignment/[assigned] to the user).
...
Correlation expression is in fact just a (parametric) search query, i.e. correlation expression is just a xref:/midpoint/reference/latest/concepts/query/xml-query-language/[search filter] with an xref:/midpoint/reference/latest/expressions/expressions/[expression] inside.
See xref:/midpoint/reference/latest/concepts/query/xml-query-language/[XML Object Query] page for more details.
...
The result simply determines xref:/midpoint/reference/latest/synchronization/situations/[synchronization situation] of the account.
...
That will result in `unlinked`xref:/midpoint/reference/latest/synchronization/situations/[synchronization situation]. The usual reaction to this situation is that such user and the account will be xref:/midpoint/reference/latest/roles-policies/assignment/assigning-vs-linking/[linked]. However that may not necessarily be the case.
