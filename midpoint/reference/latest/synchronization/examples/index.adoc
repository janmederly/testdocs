...
As the change is detected, the provisioning component makes sure the corresponding xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/[AccountShadow] object is updated in midPoint repository (see xref:/midpoint/reference/latest/schema/common-data-model/[Common Data Model]). Account Shadow usually contains only identifiers and therefore it should be already up-to-date in this case.
...
The synchronization logic in the xref:/midpoint/architecture/archive/subsystems/provisioning/[provisioning subsystem] will issue a change notification that is picked up by the xref:/midpoint/architecture/archive/subsystems/model/[IDM model]. IDM model will try to figure out what's going on (so called _xref:/midpoint/reference/latest/synchronization/situations/[situation]_, it is described in the second example below).
In this case it simply means that the IDM model will check if the account has an _owner_, that means whether it is already xref:/midpoint/reference/latest/roles-policies/assignment/assigning-vs-linking/[linked to a user]. This example assumes that it has an owner, so locating appropriate user object is straightforward.
...
That definition may also contain xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound mapping] that define how to handle change of the attribute and propagate it within midPoint.
...
Please note the `schemaHandling` definition of attribute `ri:cn`. It defines an xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound mapping] telling midPoint to copy the value of this attribute to the `fullName` property of User object.
...
But this time it looks for xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound expressions]. The resource definition for the AD resource looks like this:
...
The model looks through all the xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound expressions], looking where the change of the `fullName` can be applied.
...
Firstly it determines what's the state of the midPoint repository as compared to the new information fetched from the resource, determining a _xref:/midpoint/reference/latest/synchronization/situations/[situation]_. Secondly, it consults the synchronization policy to find out how to react to the situation.
...
It does this by using a part of the synchronization policy known as xref:/midpoint/reference/latest/synchronization/correlation-and-confirmation-expressions/[Correlation and Confirmation Expressions]. MidPoint tries to use these expressions to find a potential owner for the new account.
...
The xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound expressions] from `schemaHandling` are used for that.
...
The user template specifies that the `fullName` has to be constructed by executing an xref:/midpoint/reference/latest/expressions/expressions/script/groovy/[Groovy script expression] that will concatenate the `givenName` and `familyName` properties.
...
The template specifies that the user should have an xref:/midpoint/reference/latest/roles-policies/assignment/[assignment]. The assignment assigns an account to the user.
...
xref:/midpoint/reference/latest/roles-policies/rbac/[Role-based access control] gets processed in the _outbound_ phase.
...
See xref:/midpoint/reference/latest/synchronization/flavors/[Synchronization Flavors] page for more details.
...
* xref:/midpoint/reference/latest/concepts/query/xml-query-language/[XML Object Query]
