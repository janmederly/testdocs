...
The authorization model is seamlessly integrated with the xref:/midpoint/reference/latest/roles-policies/rbac/[RBAC mechanism]. Therefore it is easy to use even a complex authorization schemas in an IDM solution.
...
Therefore we have decided to integrate midPoint authorization model with the xref:/midpoint/reference/latest/roles-policies/rbac/[RBAC mechanism]. Authorization statements are placed inside RBAC roles.
...
It combines authorization, xref:/midpoint/reference/latest/roles-policies/rbac/[RBAC] and xref:/midpoint/reference/latest/org/organizational-structure/[organizational structure] feature to create a delegated administration set-up.
...
A user needs to have appropriate xref:/midpoint/reference/latest/security/authorization/admin-gui/[GUI authorization] to access the relevant GUI page.
...
* *Request* authorization is evaluated for each operation of xref:/midpoint/reference/latest/interfaces/model-java/[IDM Model Interface] before the operation starts.
...
This authorization is evaluated *before* the object is xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[recomputed by the IDM model] (i.e. before all the mappings and policies are applied).
Properties that are result of the computation are *not* yet present in the object.

* *Execution* authorization is evaluated for each operation of xref:/midpoint/reference/latest/interfaces/model-java/[IDM Model Interface] *after* the object is xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[recomputed by the IDM model] and right before the operation is executed.
...
See the xref:/midpoint/reference/latest/security/authorization/admin-gui/[GUI Authorizations] page for a full list of supported GUI actions.
See the xref:/midpoint/reference/latest/security/authorization/service/[Service Authorizations] page for similar list of service authorizations.
...
It does all the policy processing, evaluates mappings, recomputes objects, xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[projects values between objects] and does all the other things of identity management logic.
...
This applies to all normal operations which includes operations initiated from GUI and all the remote services (SOAP, REST) as all of these components are using the xref:/midpoint/reference/latest/interfaces/model-java/[IDM Model Interface]. As this interface is used almost universally in midPoint the action URIs used for authorization are also based on the operation names of the xref:/midpoint/reference/latest/interfaces/model-java/[IDM Model Interface] - with some minor adjustments to make them practical.

See the xref:/midpoint/reference/latest/security/authorization/model/[IDM Model Authorizations] page for list of action URLs for the "core" authorizations.
...
The xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[Clockwork and Projector] page explains the details.
...
This mapping is implemented by simple xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mappings]. If the `familyName` of a user is changed in GUI then this change is also mapped to the LDAP `sn` attribute and this is changed as well.
...
It is important to realize that change of one object can result in a change of a different object, e.g. as xref:/midpoint/reference/latest/schema/focus-and-projections/[data are mapped between focus and projections]. And authorizations needs to be set up accordingly.
...
But if the user learns the OID from some other channel, the user may trick the system or even abuse xref:/midpoint/reference/latest/interfaces/[midPoint interfaces] to gain access to an object that he should not be accessing.
...
See also xref:/midpoint/reference/latest/schema/archetypes/[Archetypes].
...
The object owner is its xref:/midpoint/reference/latest/schema/focus-and-projections/[focal object]. E.g. typical owner of account shadows is a user to whom the accounts are linked.
...
This has important implications especially for xref:/midpoint/reference/latest/deployment/multitenancy/[multitenant deployments].
...
The target is the object which is assigned or unassigned (role, org, service or xref:/midpoint/reference/latest/misc/deputy/[deputy user]). This can be used to precisely control which objects may be assigned or unassigned.
...
Order of one or more means inducement (see xref:/midpoint/reference/latest/roles-policies/metaroles/gensync/['Roles, Metaroles and Generic Synchronization'] page for more details).
...
xref:/midpoint/reference/latest/expressions/expressions/[Expressions] can be used in authorization search filters:
...
If you need complex computation, it is perhaps better to compute the value in xref:/midpoint/reference/latest/expressions/object-template/[object template] and place it into property of the object (e.g. user extension property).
...
Main article: xref:/midpoint/reference/latest/diag/troubleshooting/authorizations/[Troubleshooting Authorizations]
...
In this mode midPoint will trace processing of all authorization statements and record that in the xref:/midpoint/reference/latest/diag/logging/[logfiles]. The trace can be enabled by setting the following log levels:
...
See xref:/midpoint/reference/latest/diag/troubleshooting/authorizations/[Troubleshooting Authorizations] for more details.
...
* xref:/midpoint/reference/latest/diag/troubleshooting/authorizations/[Troubleshooting Authorizations]

* xref:/midpoint/reference/latest/security/authorization/admin-gui/[GUI Authorizations]

* xref:/midpoint/reference/latest/security/authorization/service/[Service Authorizations]

* xref:/midpoint/reference/latest/security/authorization/model/[IDM Model Authorizations]

* xref:/midpoint/reference/latest/roles-policies/rbac/[Advanced Hybrid RBAC]
