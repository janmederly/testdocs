...
:page-alias: [ { "parent" : "/midpoint/features/current/" }, { "parent" : "/midpoint/reference/v2/security/" } ]
...
MidPoint xref:/midpoint/reference/v2/expressions/expressions/[expressions] are a very flexible tool that can be used to customize midPoint behavior almost to the extreme.
Expressions are used everywhere: xref:/midpoint/reference/v2/expressions/mappings/[mappings], role conditions, reports and all the way up to user interface customization.
...
Expressions, and especially xref:/midpoint/reference/v2/expressions/expressions/script/[scripting expressions], can do anything.
...
Even though the dangers are quite obvious and self-explanatory, we have xref:/midpoint/reference/v2/security/security-guide/[warned] the users about this all the time.
...
* xref:/midpoint/reference/v2/roles-policies/metaroles/policy/[Metaroles] can be used to group all the necessary logic, including expressions.
...
* xref:/midpoint/reference/v2/concepts/object-lifecycle/[Object lifecycle] mechanism can be used to drive role definitions through an approval process.
...
Expression profiles are defined in xref:/midpoint/reference/v2/concepts/system-configuration-object/[system configuration.] That part is quite easy.
...
Therefore the use of expression profiles is integrated with xref:/midpoint/reference/v2/schema/archetypes/[archetypes]. Object archetype assignment does not usually change - and if it does it is quite an unusual and perhaps very special process.
