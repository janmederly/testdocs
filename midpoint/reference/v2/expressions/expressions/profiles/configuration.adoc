...
See xref:/midpoint/reference/v2/expressions/expressions/profiles/[Expression Profiles] page for a generic introduction to expression profile concepts.

== Expression Profile Specification

Expression profiles are defined in xref:/midpoint/reference/v2/concepts/system-configuration-object/[system configuration]:
...
Please see the xref:/midpoint/reference/v2/misc/bulk/index.adoc#_actions[list of all actions].
...
There is an option to evaluate an expression either with elevated privileges, or under a different identity (see xref:/midpoint/reference/v2/security/privilege-elevation.adoc[]).
...
The primary usage pattern for expression profiles is in conjunction with xref:/midpoint/reference/v2/schema/archetypes/[archetypes].
...
For more comprehensive example, please see xref:/midpoint/reference/v2/security/trusted-actions/index.adoc[].

=== Default Object Policy Configuration

If archetype(s) for a given object do not point to an expression profile, midPoint looks at `defaultObjectPolicyConfiguration` in xref:/midpoint/reference/v2/concepts/system-configuration-object/[system configuration].
...
This configuration is xref:/midpoint/reference/v2/security/security-guide/[not meant to be completely secure].
