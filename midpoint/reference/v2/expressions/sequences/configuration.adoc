...
The values are kept in xref:/midpoint/reference/v2/concepts/clockwork/model-context/[Model Context] and re-used every time the expression is re-evaluated during that operation.
...
See xref:/midpoint/reference/v2/samples/story-tests/unix/[Unix Story Test] for a complete example that is using sequences.
