...
However, there is a special language that is used to reference items inside prism objects: xref:/midpoint/reference/v2/concepts/item-path/[item path]. This language is designed to process the paths in a very efficient way, and it is used at many places in midPoint.
...
| `user` (deprecated)
| Represents xref:/midpoint/reference/v2/schema/focus-and-projections/[focal object] which is usually a user.
...
| Represents xref:/midpoint/reference/v2/schema/focus-and-projections/[projection] in a form of xref:/midpoint/reference/v2/resources/shadow/[shadow].
...
The xref:/midpoint/reference/v2/schema/custom-schema-extension/[extension] of system configuration may be used to hold system-wide configuration and constants.
...
|
| Numeric value describing the current xref:/midpoint/reference/v2/concepts/iteration/[iteration].
...
| object template, outbound
|
| String value describing the current xref:/midpoint/reference/v2/concepts/iteration/[iteration].
...
The projection is usually legal if there is an xref:/midpoint/reference/v2/roles-policies/assignment/[assignment] for it.
But the projection may also be legal without an assignment, e.g. if xref:/midpoint/reference/v2/synchronization/projection-policy/[assignment policy enforcement] is set to NONE.
...
| outbound
|
| Contains a definition of the association target (xref:/midpoint/reference/v2/resources/entitlements/[entitlement]). Used in expressions that need to do advanced logic on associations and entitlements.
...
E.g. the variables specific to assignment processing are described in the xref:/midpoint/reference/v2/roles-policies/assignment/configuration/[Assignment Configuration] page.
...
This is a long time ago and midPoint is now very xref:/midpoint/reference/v2/synchronization/generic-synchronization/[generic].
...
Please xref:/midpoint/reference/v2/diag/creating-a-bug-report/[report the bug].
We will fix that.

== See Also

* xref:/midpoint/reference/v2/expressions/expressions/[Expression]

* xref:/midpoint/reference/v2/expressions/mappings/[Mapping]
