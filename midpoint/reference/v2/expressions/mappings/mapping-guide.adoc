...
Because expressions are so flexible they are documented in a separate xref:/midpoint/reference/v2/expressions/expressions/[Expression] page.
...
The target definition is necessary so the mapping can locate appropriate definition of the target property and therefore make sure that the expression produces a correct data type (e.g. string vs xref:/midpoint/reference/v2/concepts/polystring/[polystring] and that other schema constraints are maintained (e.g. single vs multiple values).
...
Some parts of the mapping may be implicitly defined by the surrounding context, e.g. the target is implicit in xref:/midpoint/reference/v2/expressions/mappings/outbound-mapping/[outbound mapping definition]. Similarly a source is implicitly defined in xref:/midpoint/reference/v2/expressions/mappings/inbound-mapping/[inbound mapping]. Therefore for outbound and inbound mappings it is usually sufficient to define either source or target:
