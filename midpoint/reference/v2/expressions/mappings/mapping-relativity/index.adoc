...
However, variables are usually too simple to handle the xref:/midpoint/reference/v2/concepts/relativity/[relative change model] used in midPoint.
...
This is the functionality of midPoint xref:/midpoint/reference/v2/expressions/mappings/[mappings].
...
MidPoint mappings are designed to work correctly with the xref:/midpoint/reference/v2/concepts/relativity/[relative change model] that is a fundamental principle of midPoint operation.
...
There is also a convenient xref:/midpoint/reference/v2/expressions/expressions/script/functions/[script function]`basic.isEmpty()` that can provide null-safe check for empty value:
...
Mapping output is used to create output deltas, which are applied to output properties in the final steps of xref:/midpoint/reference/v2/concepts/clockwork/clockwork-and-projector/[clockwork processing].
...
This means that there are often _triples of triples_ in xref:/midpoint/reference/v2/concepts/clockwork/clockwork-and-projector/[projector computation].
...
As a principle, existing values of target property are processed by xref:/midpoint/reference/v2/concepts/clockwork/clockwork-and-projector/[projector] only if they are available, only if they were already fetched.
...
While midPoint is built with xref:/midpoint/reference/v2/concepts/relativity/[Relativity] in mind, this relativity is not complete.
