...
(Unless the scheduler on that node is stopped, of course.) The usual size of the pool is 10 threads, but can be configured to any other value using xref:/midpoint/reference/v2/tasks/task-manager/configuration/[task manager configuration]. Besides this configuration, a node can declare which execution groups it accepts, and for how many threads.
