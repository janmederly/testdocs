...
Any xref:/midpoint/reference/v2/tasks/activities/[activity] in midPoint can execute in two basic modes: _persistent-effects_ or _simulation_.
...
[NOTE]
====
There are some specialties like postponing resource operations either because of the resource unavailability (planned - see xref:/midpoint/reference/v2/resources/maintenance-state/[maintenance mode] - or unplanned), or because of xref:/midpoint/reference/v2/resources/propagation/[provisioning propagation].
...
The changes are computed, just like in <<Persistent-Effects Mode>>, but instead of their application, they are written to so-called xref:/midpoint/reference/v2/simulation/results/[simulation result].
...
NOTE: Please see the xref:/midpoint/reference/v2/simulation/tutorial/[tutorial] where these scenarios are shown in more detail.
...
The simulations are typically executed in background tasks that host individual xref:/midpoint/reference/v2/tasks/activities/[activities].
...
Please see the xref:/midpoint/reference/v2/simulation/tutorial/[tutorial] for an example.

This configuration is driven by the `execution/mode` parameter in the activity definition.

For more information, please see the xref:/midpoint/reference/v2/tasks/activities/execution-mode.adoc[activity execution mode specification].
...
For more information, please see xref:/midpoint/reference/v2/tasks/activities/execution-mode.adoc[activity execution mode specification].
...
For more information, please see <<Configuring Simulation Results>> and xref:/midpoint/reference/v2/tasks/activities/reporting/simulation-result.adoc[activity simulation result definition].
...
Simulation results can be displayed interactively via the midPoint GUI, or their content can be exported using the xref:/midpoint/reference/v2/misc/reports/configuration/[reporting] functionality.
...
_Simulated actions are executed using the same xref:/midpoint/reference/v2/concepts/clockwork/clockwork-and-projector/[clockwork] as persistent-effect ones.
...
. xref:/midpoint/reference/v2/expressions/expressions/index.adoc#_create_on_demand[Create-on-demand] scenarios are supported in a very limited way:
