...
The feature is called _iteration_ and it can be enabled for any resource in xref:/midpoint/reference/v2/resources/resource-configuration/schema-handling/[Resource Schema Handling]. Two steps are required to configure this feature:

* Enable the iteration by setting the maximum number of iteration attempts.
This is done by setting the `iteration` property in the xref:/midpoint/reference/v2/resources/resource-configuration/schema-handling/[Resource Schema Handling].
