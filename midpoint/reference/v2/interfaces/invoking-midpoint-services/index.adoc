...
But midPoint also exposes all its functionality using xref:/midpoint/reference/v2/interfaces/[midPoint interfaces].
...
The key to midPoint flexibility are the xref:/midpoint/reference/v2/interfaces/[interfaces].
...
All the versions of midPoint xref:/midpoint/reference/v2/interfaces/model-java/[IDM Model Interface] are object-oriented.
It means that they work with xref:/midpoint/reference/v2/schema/data-model-essentials/[midPoint objects].
...
All the interfaces are also using the xref:/midpoint/reference/v2/concepts/relativity/[relative change model] by using xref:/midpoint/devel/prism/concepts/deltas/[deltas] to define change to individual midPoint objects.
...
Main article: xref:/midpoint/reference/v2/interfaces/model-java/[IDM Model Interface]

xref:/midpoint/reference/v2/interfaces/model-java/[IDM Model Interface] is a local Java interface to most midPoint functionality.
...
* xref:/midpoint/reference/v2/interfaces/model-java/introduction/[IDM Model Interface Introduction]

* *TODO*: example usage from scripts

== Web Service Interface

Main article: xref:/midpoint/reference/v2/legacy/soap/[IDM Model Web Service Interface]

xref:/midpoint/reference/v2/legacy/soap/[IDM Model Web Service Interface] is a SOAP-based web service interface to midPoint.
It provides operations that are very similar to Java-based xref:/midpoint/reference/v2/interfaces/model-java/[IDM Model Interface], however there are some necessary adjustments.
The xref:/midpoint/reference/v2/legacy/soap/[IDM Model Web Service Interface] is strongly RPC-based as this approach is dictated by the technology (SOAP, WSDL).
...
There is also a xref:/midpoint/reference/v2/legacy/soap-client/[Model Web Service Client] library based on Java and pure JAX-WS and JAX-B.
...
* xref:/midpoint/reference/v2/legacy/soap-client/sample/[Model Web Service Client Sample] describes a sample code which is part of midPoint source code.
This sample code is using model client library to invoke midPoint web service.

Note: xref:/midpoint/reference/v2/legacy/soap/[IDM Model Web Service Interface] is still "version 1" interface.
...
Main article: xref:/midpoint/reference/v2/interfaces/rest/[REST API]

MidPoint xref:/midpoint/reference/v2/interfaces/rest/[REST] interface is a RESTful HTTP-based interface.
...
However informal documentation is (will be) available and there is also a xref:/midpoint/reference/v2/schema/[schema] that describes the structure of individual objects.

== See Also

* xref:/midpoint/reference/v2/interfaces/[Interfaces]

* xref:/midpoint/architecture/[Architecture and Design]

* xref:/midpoint/reference/v2/interfaces/model-java/[IDM Model Interface]

* xref:/midpoint/reference/v2/legacy/soap/[IDM Model Web Service Interface]
