...
* *xref:/midpoint/reference/v2/roles-policies/rbac/[Roles]* are used to define _provisioning_ and _access control_ schemes.
...
Roles can be xref:/midpoint/reference/v2/roles-policies/rbac/[very flexible and dynamic]. There can be hierarchical roles, conditional roles, parametric roles, etc.
Generally speaking roles can contain xref:/midpoint/reference/v2/expressions/[mappings and expressions] to create a very dynamic structures.
Roles can be personalized for each particular user, e.g. by using xref:/midpoint/reference/v2/roles-policies/assignment/[assignment parameters].

* *xref:/midpoint/reference/v2/misc/services/[Services]* are used to represent servers, mobile devices, printers, virtual machines, "things" (as in IoT) and other objects whose are not entirely users, orgs or roles.
...
* *xref:/midpoint/reference/v2/org/organizational-structure/[Orgs]* are used to _group_ users or other objects.
...
And this may depend on the way how the role is xref:/midpoint/reference/v2/roles-policies/assignment/[assigned] to a particular user.
...
This is what xref:/midpoint/reference/v2/concepts/clockwork/clockwork-and-projector/[Clockwork and Projector] components do and it is quite a demanding calculation.
...
Simply speaking it can have xref:/midpoint/reference/v2/roles-policies/assignment/assignment-vs-inducement/[inducements]. This can be used for multitude of purposes, e.g. to "automatically assign" roles for members of particular organizational unit or project.
