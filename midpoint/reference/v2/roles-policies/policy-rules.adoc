...
It can be used to define role xref:/midpoint/reference/v2/cases/approval/[approval], xref:/midpoint/reference/v2/roles-policies/role-lifecycle/[lifecycle], policies such as xref:/midpoint/reference/v2/roles-policies/segregation-of-duties/[segregation of duties], notifications and other governance and compliance rules.
...
* *xref:/midpoint/reference/v2/schema/focus-and-projections/[Focal object]* that the rule applies to.
...
This is used when applying the rule to the xref:/midpoint/reference/v2/roles-policies/assignment/[assignment].
...
The actions may range from strong enforcement of the rule (i.e. prohibit the operation) through xref:/midpoint/reference/v2/cases/approval/[Approval] to a very mild actions such as notifications.

Some of these parts may be implicit.
E.g. if the rule is defined as a xref:/midpoint/reference/v2/roles-policies/metaroles/gensync/[high-order inducement in the metarole] then the focal object or target are implicit and does not need to be explicitly specified.

The policy rules are used to implement wide variety of mechanisms:

* xref:/midpoint/reference/v2/cases/approval/[Assignment approvals]: policy rules may be used to change the default assignment approval schemes in a very flexible and scalable fashion.

* xref:/midpoint/reference/v2/roles-policies/role-lifecycle/[Role lifecycle management]: policy rules may be used to enforce approvals of new roles before they are used in production.
...
Policy rules can be defined in xref:/midpoint/reference/v2/concepts/system-configuration-object/[system configuration object].
...
Which means they can be taken to the meta-level using the usual midPoint mechanism of xref:/midpoint/reference/v2/roles-policies/metaroles/gensync/[metaroles].
...
See the xref:/midpoint/reference/v2/roles-policies/metaroles/gensync/['Roles, Metaroles and Generic Synchronization'] page for more details about the meta-role mechanism.
...
Common use case is an exception from xref:/midpoint/reference/v2/roles-policies/segregation-of-duties/[Segregation of Duties] policy: the policy states that certain roles cannot be combined.
...
However even in midPoint 3.5 this feature was considered *EXPERIMENTAL* except when used for xref:/midpoint/reference/v2/roles-policies/role-lifecycle/[Role Lifecycle] and assignment-based xref:/midpoint/reference/v2/cases/approval/[approvals].
...
* xref:/midpoint/reference/v2/cases/approval/[Approval]

* xref:/midpoint/reference/v2/roles-policies/segregation-of-duties/[Segregation of Duties]

* xref:/midpoint/reference/v2/roles-policies/rbac/radio-button-roles/[Radio Button Roles]
