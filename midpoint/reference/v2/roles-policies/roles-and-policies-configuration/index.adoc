...
Users are assigned to the roles using a mechanism called xref:/midpoint/reference/v2/roles-policies/assignment/[assignment] (see below).
...
xref:/midpoint/reference/v2/expressions/[Mappings and expressions] are used to define dynamic implied account attributes.
xref:/midpoint/reference/v2/concepts/item-path/[Simple paths] are perhaps the most easy to use and most useful in common situations.
There are also xref:/midpoint/reference/v2/expressions/expressions/script/[scripting expressions] for more complex cases.
...
In fact the role definitions are using xref:/midpoint/reference/v2/expressions/mappings/[mappings] to determine attribute values.
It is the same mechanism that is used in xref:/midpoint/reference/v2/roles-policies/assignment/[assignments] and xref:/midpoint/reference/v2/resources/resource-configuration/schema-handling/[resource schema handling section] therefore the same features and limitations apply here.
Following diagram provides more detailed illustration of use of mappings in the roles.
Each mapping has three parts: source, value constructor and target (see xref:/midpoint/reference/v2/expressions/mappings/[Mapping]). However some of these parts can be determined by the context in which the mapping is used.
...
Please see the xref:/midpoint/reference/v2/expressions/[Mappings and Expressions] page for explanation of basic principles of mapping mechanism.
...
The account may also have existing attributes that are managed by "native" tools (outside IDM) or there may be exceptions from the RBAC policy specified for that account using attribute specification in xref:/midpoint/reference/v2/roles-policies/assignment/[assignments].
...
Please see xref:/midpoint/reference/v2/resources/entitlements/[] page for the details.

== Assignments

Main article: xref:/midpoint/reference/v2/roles-policies/assignment/[Assignment], xref:/midpoint/reference/v2/roles-policies/assignment/configuration/[Assignment Configuration]
...
See xref:/midpoint/reference/v2/roles-policies/assignment/[Assignment] and xref:/midpoint/reference/v2/roles-policies/assignment/configuration/[Assignment Configuration] pages for more details about assignments.

== Inducements

Main article: xref:/midpoint/reference/v2/roles-policies/assignment/assignment-vs-inducement/[Assignment vs Inducement]
...
See xref:/midpoint/reference/v2/roles-policies/assignment/assignment-vs-inducement/[Assignment vs Inducement] for more details.
...
See xref:/midpoint/reference/v2/roles-policies/roles-services-and-orgs/['Roles, Services and Orgs']

== Meta-Roles

See xref:/midpoint/reference/v2/roles-policies/metaroles/gensync/['Roles, Metaroles and Generic Synchronization']
...
* xref:/midpoint/reference/v2/roles-policies/assignment/configuration/[Assignment Configuration]

* xref:/midpoint/reference/v2/roles-policies/assignment/assignment-vs-inducement/[Assignment vs Inducement]

* xref:/midpoint/reference/v2/roles-policies/roles-services-and-orgs/['Roles, Services and Orgs']

* xref:/midpoint/reference/v2/roles-policies/rbac/best-practice/[RBAC Best Practice]
