...
The role above will be automatically assigned to any user that has property `employeeType` set to `intern`. As all midPoint mapping even this mapping is xref:/midpoint/reference/v2/concepts/relativity/[relativistic]. If user becomes an intern (the `employeeType` property is changed) then the role is automatically assigned.
When the user stops being an intern the role is unassigned.

See xref:/midpoint/reference/v2/roles-policies/role-autoassignment/configuration/[Role Autoassign Configuration] page for more details.

== Autoassignment in Object Template

Autoassignment in roles is simple and elegant, but it has several disadvantages.
However, even complex role autoassignment cases can be handled by using xref:/midpoint/reference/v2/expressions/object-template/[object template]. The roles can be automatically assigned by using object template mappings:
...
See xref:/midpoint/reference/v2/expressions/object-template/[Object Template] page for more details about object template mechanism.
The xref:/midpoint/reference/v2/expressions/expressions/[Expression] page describes the details of `assignmentTargetSearch` expression evaluator that is frequently used for this purpose.

== Roles Within Roles

There is sometimes a need to assign one role when another role is assigned.
As midPoint has full support for xref:/midpoint/reference/v2/roles-policies/rbac/[role hierarchy] this is easily done by nesting the roles inside.
...
xref:/midpoint/reference/v2/roles-policies/segregation-of-duties/[Role exclusion] mechanism may be used to implement this approach, as illustrated by xref:/midpoint/reference/v2/roles-policies/rbac/radio-button-roles/[Radio Button Roles] example.
...
* xref:/midpoint/reference/v2/expressions/object-template/[Object Template]
