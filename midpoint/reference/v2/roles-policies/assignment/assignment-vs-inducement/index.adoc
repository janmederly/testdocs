...
They are used to assign roles, organizational units, accounts, entitlements and other things to xref:/midpoint/reference/v2/schema/focus-and-projections/[focal objects]. Both assignment and inducement has the same internal structure that is described in xref:/midpoint/reference/v2/roles-policies/assignment/configuration/[Assignment Configuration] page.
...
But the situation gets very complex once we allow roles and organizational units to have associated _shadows_ in a similar way that user has account _shadow_ (a.k.a. xref:/midpoint/reference/v2/synchronization/generic-synchronization/[Generic Synchronization]). This is a very powerful feature.
...
You want to deviate from this rule usually only if xref:/midpoint/reference/v2/synchronization/generic-synchronization/[Generic Synchronization] is used.
