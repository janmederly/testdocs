...
xref:/midpoint/reference/v2/tasks/task-manager/[Clustering / high availability] can be achieved by setting up several midPoint nodes working against common midPoint repository.
...
Alternatively, you can use the xref:/midpoint/reference/v2/deployment/midpoint-home-directory/overriding-config-xml-parameters.adoc[command line options] `-Dkey=value` to set these parameters (*Command-line parameter* in the table below).
...
(See xref:/midpoint/reference/v2/repository/configuration/[Repository Configuration]).
