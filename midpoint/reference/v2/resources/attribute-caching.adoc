...
It can be turned on in xref:/midpoint/reference/v2/resources/resource-configuration/[Resource Configuration]:
...
If caching is turned on then midPoint will start to cache the data in xref:/midpoint/reference/v2/resources/shadow/[Shadow Objects]. The caches are build gradually as midPoint reads the objects.
...
The cached data are accessible by using the ususal xref:/midpoint/reference/v2/interfaces/model-java/[IDM Model Interface]. There are two operation options that provide access to the cached data:
...
xref:/midpoint/reference/v2/resources/shadow/[Shadow Objects] contain `cachingMetadata` property.
