...
The example of such action is _Request access_ button in _My accesses_ preview widget, which will take a user to the xref:/midpoint/reference/v2/admin-gui/request-access[access request] process.
...
It is also possible to use midPoint's standard xref:/midpoint/reference/v2/admin-gui/admin-gui-config/#how-it-works[merging mechanism] to override widgets visibility, actions, etc. with configuration in roles, archetypes or user's profile.
...
<8> Defines actual action which is performed after clicking on the button. In this concrete example, user will be redirected to the xref:/midpoint/reference/v2/admin-gui/request-access[Request access] process.
...
To configure self-service profile, the same rules and same panels can be used as described in xref:/midpoint/reference/v2/admin-gui/admin-gui-config/#object-details[object details] page.
