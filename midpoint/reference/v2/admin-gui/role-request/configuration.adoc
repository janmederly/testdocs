...
The prerequisite for a good user experience is usually proper configuration of xref:/midpoint/reference/v2/admin-gui/role-catalog/configuration/[role catalog] and xref:/midpoint/reference/v2/security/authorization/[authorizations]. The end-user role selection page will display only those roles that the currently-logged-in user can assign to himself.
...
This can be controlled in the xref:/midpoint/reference/v2/concepts/system-configuration-object/[system configuration object] like this:
...
Default assignment constraints are specified in xref:/midpoint/reference/v2/concepts/system-configuration-object/[system configuration object]. These constraints are applied globally to the entire system.
