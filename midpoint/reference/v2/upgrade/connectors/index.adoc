...
These connectors must be downloaded separately and installed in xref:/midpoint/reference/v2/deployment/midpoint-home-directory/[midPoint home directory] or in a xref:/connectors/connid/1.x/connector-server/[connector server]. The connectors are not bundled with midPoint for many reasons.
...
* xref:/midpoint/reference/v2/resources/connector-setup/[Connector Setup]
