...
A user needs to have appropriate xref:/midpoint/reference/v2/security/authorization/admin-gui/[GUI authorization] to access the relevant GUI page.
...
* *Request* authorization is evaluated for each operation of xref:/midpoint/reference/v2/interfaces/model-java/[IDM Model Interface] before the operation starts.
...
This authorization is evaluated *before* the object is xref:/midpoint/reference/v2/concepts/clockwork/clockwork-and-projector/[recomputed by the IDM model] (i.e. before all the mappings and policies are applied).
Properties that are result of the computation are *not* yet present in the object.

* *Execution* authorization is evaluated for each operation of xref:/midpoint/reference/v2/interfaces/model-java/[IDM Model Interface] *after* the object is xref:/midpoint/reference/v2/concepts/clockwork/clockwork-and-projector/[recomputed by the IDM model] and right before the operation is executed.
...
See the xref:/midpoint/reference/v2/security/authorization/admin-gui/[GUI Authorizations] page for a full list of supported GUI actions.
See the xref:/midpoint/reference/v2/security/authorization/service/[Service Authorizations] page for similar list of service authorizations.
...
It does all the policy processing, evaluates mappings, recomputes objects, xref:/midpoint/reference/v2/concepts/clockwork/clockwork-and-projector/[projects values between objects] and does all the other things of identity management logic.
...
This applies to all normal operations which includes operations initiated from GUI and all the remote services (REST, SOAP) as all of these components are using the xref:/midpoint/reference/v2/interfaces/model-java/[IDM Model Interface].
As this interface is used almost universally in midPoint the action URIs used for authorization are also based on the operation names of the xref:/midpoint/reference/v2/interfaces/model-java/[IDM Model Interface] - with some minor adjustments to make them practical.

See the xref:/midpoint/reference/v2/security/authorization/model/[IDM Model Authorizations] page for list of action URLs for the "core" authorizations.
...
The xref:/midpoint/reference/v2/concepts/clockwork/clockwork-and-projector/[Clockwork and Projector] page explains the details.
...
This mapping is implemented by simple xref:/midpoint/reference/v2/expressions/mappings/outbound-mapping/[outbound mappings].
...
It is important to realize that change of one object can result in a change of a different object, e.g. as xref:/midpoint/reference/v2/schema/focus-and-projections/[data are mapped between focus and projections].
...
A typical example of such instantiation is their use for xref:/midpoint/reference/v2/admin-gui/admin-gui-config/#custom-actions-for-object-lists[custom GUI actions for object lists].
...
But if the user learns the OID from some other channel, the user may trick the system or even abuse xref:/midpoint/reference/v2/interfaces/[midPoint interfaces] to gain access to an object that he should not be accessing.
...
This has important implications especially for xref:/midpoint/reference/v2/deployment/multitenancy/[multitenant deployments].
...
The target is the object which is assigned or unassigned (role, org, service or xref:/midpoint/reference/v2/misc/deputy/[deputy user]).
...
Order of one or more means inducement (see xref:/midpoint/reference/v2/roles-policies/metaroles/gensync/['Roles, Metaroles and Generic Synchronization'] page for more details).
...
Main article: xref:/midpoint/reference/v2/diag/troubleshooting/authorizations/[Troubleshooting Authorizations]
...
In this mode midPoint will trace processing of all authorization statements and record that in the xref:/midpoint/reference/v2/diag/logging/[logfiles].
...
See xref:/midpoint/reference/v2/diag/troubleshooting/authorizations/[Troubleshooting Authorizations] for more details.
...
* xref:/midpoint/reference/v2/diag/troubleshooting/authorizations/[Troubleshooting Authorizations]

* xref:/midpoint/reference/v2/security/authorization/admin-gui/[GUI Authorizations]

* xref:/midpoint/reference/v2/security/authorization/service/[Service Authorizations]

* xref:/midpoint/reference/v2/security/authorization/model/[IDM Model Authorizations]
