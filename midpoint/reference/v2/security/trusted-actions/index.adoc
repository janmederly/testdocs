...
=== Problem
The xref:/midpoint/reference/v2/admin-gui/admin-gui-config/#custom-actions-for-object-lists[custom GUI actions for object lists] are currently implemented using task templates.
...
* xref:/midpoint/reference/v2/expressions/expressions/profiles/configuration/[Expression Profile Configuration]
