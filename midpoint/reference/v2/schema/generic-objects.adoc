...
| Role
| xref:/midpoint/reference/v2/roles-policies/rbac/[RBAC roles] +
xref:/midpoint/reference/v2/security/authorization/roles/[Authorization roles] +
...
| Org
| xref:/midpoint/reference/v2/org/organizational-structure/[Functional organization structure] (company, division, section) +
...
xref:/midpoint/reference/v2/roles-policies/applicable-policies/[Policy category] +
xref:/midpoint/reference/v2/admin-gui/role-catalog/[Role catalog category]
...
* xref:/midpoint/reference/v2/roles-policies/role-autoassignment/[Autoassignment] works as expected.
As do authorizations.
xref:/midpoint/reference/v2/cases/approval/[Approval] processes can be applied seamlessly.
...
Job positions may have an owner and they may be subject to xref:/midpoint/reference/v2/roles-policies/certification/[certification].
...
And of course, all the object types can be xref:/midpoint/reference/v2/synchronization/generic-synchronization/[synchronized with resource objects].
...
But the definition of RoleType can be extended, even at run-time, by using xref:/midpoint/reference/v2/schema/custom-schema-extension/[custom schema extension mechanism].

There is yet another drawback: RBAC roles, authorization roles, job titles - they all have the same object type.
They are all `RoleType`. How to tell them apart? There is also a mechanism for that: xref:/midpoint/reference/v2/schema/archetypes/[Archetypes]. Archetypes can be used to distinguish objects of the same type, customize its presentation and behavior.
Archetypes are also xref:/midpoint/reference/v2/roles-policies/metaroles/policy/[metaroles]. Metaroles are roles that are applied to other (abstract) roles.
...
Users, roles, orgs, services, resources, tasks - and even xref:/midpoint/reference/v2/concepts/system-configuration-object/[system configuration object]. System configuration object extension is in fact a very useful mechanism.
Most xref:/midpoint/reference/v2/expressions/[expressions] have `configuration` variable available during expression evaluation.
...
You will need to define xref:/midpoint/reference/v2/schema/custom-schema-extension/[Custom Schema Extension] for `GenericObjectType` to make it work properly.
...
* xref:/midpoint/reference/v2/schema/subtype/[Subtype]
