...
Those databases will be supported for some time (possibly long time), but xref:/midpoint/reference/latest/repository/repository-database-support/[support for them will be eventually removed].
...
* SOAP-based xref:/midpoint/reference/latest/legacy/soap/[IDM Model Web Service Interface] is deprecated, and it will be removed in midPoint 4.2. Please use xref:/midpoint/reference/latest/interfaces/rest/[RESTful interface] instead.
...
* Property `publicHttpUrlPattern` is used in xref:/midpoint/reference/latest/concepts/system-configuration-object/[System Configuration Object] to create links in notifications.
