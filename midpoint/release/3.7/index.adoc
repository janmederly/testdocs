...
* xref:/midpoint/reference/latest/deployment/stand-alone-deployment/[Stand-alone deployment] based on Spring Boot
...
** Significantly improved xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound mapping]

** Selection of assignment path index in `associationFromLink` expressions.

** Function to determine projection existence

** xref:/midpoint/reference/latest/misc/bulk/variables/[Variables in bulk actions]

** xref:/midpoint/reference/latest/roles-policies/role-autoassignment/[Role autoassignment]
...
** xref:/midpoint/reference/latest/security/credentials/password-policy/[Password policy] improvements to enforce different persona passwords.
...
MidPoint 3.7 data model is essentially backwards compatible with both midPoint 3.6 and midPoint 3.6.1. However as the data model was extended in 3.7 the database schema needs to be upgraded using the xref:/midpoint/reference/latest/upgrade/database-schema-upgrade/[usual mechanism]. There are a few points to highlight that are related to database structure upgrade:
...
* MidPoint has switched to a xref:/midpoint/reference/latest/deployment/stand-alone-deployment/[stand-alone deployment] model.
...
** The location of a default xref:/midpoint/reference/latest/deployment/midpoint-home-directory/[midPoint home directory] has changed.
...
The default log file was changed from `idm.log` to `midpoint.log`. However, please note that if you have existing logging configuration in xref:/midpoint/reference/latest/concepts/system-configuration-object/[system configuration object], that configuration will be still used after upgrade.
...
* xref:/midpoint/reference/latest/deployment/stand-alone-deployment/[Stand-Alone Deployment] is now the default deployment model (see above).
...
* There are new xref:/midpoint/reference/latest/security/authorization/configuration/[authorizations] for raw operations and partial execution operations.
...
Brief documentation is available at xref:/midpoint/reference/latest/deployment/ninja/[Ninja] page.
...
Now the entire xref:/midpoint/reference/latest/roles-policies/assignment/using-assignment-path-in-expressions/[assignment path] is exposed for use in the expressions.
