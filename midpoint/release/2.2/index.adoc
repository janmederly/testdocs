...
* xref:/midpoint/architecture/archive/subsystems/repo/identity-repository-interface/[Flexible identity repository implementations] and xref:/midpoint/reference/latest/repository/generic/implementation/[SQL repository implementation]
...
** xref:/midpoint/reference/latest/expressions/expressions/script/xpath/[XPath version 2] and xref:/midpoint/reference/latest/legacy/xpath2/[XPath Tutorial]
...
* Rule-based RBAC (RB-RBAC) ability by using conditional mappings in xref:/midpoint/reference/latest/expressions/object-template/[user template]

* Basic xref:/midpoint/reference/latest/security/audit/[auditing]
...
* Simple xref:/midpoint/reference/latest/synchronization/consistency/[handling of provisioning errors]

* xref:/midpoint/reference/latest/resources/resource-configuration/protected-accounts/[Protected accounts] (accounts that will not be affected by midPoint)

* xref:/midpoint/reference/latest/roles-policies/segregation-of-duties/[Segregation of Duties] (SoD)

** xref:/midpoint/reference/latest/roles-policies/segregation-of-duties/[Role exclusions]
...
* xref:/midpoint/reference/latest/cases/workflow-3/[Workflow support] (based on link:http://www.activiti.org/[Activiti])
...
* xref:/midpoint/reference/latest/cases/workflow-3/[Production-quality workflow integration] (using Activiti)

* xref:/midpoint/reference/latest/security/authorization/[Authorizations for GUI] and web service integrated into RBAC mechanism
...
* Numerous xref:/midpoint/reference/latest/resources/resource-configuration/schema-handling/activation/[activation] enhancements

** Redesigned xref:/midpoint/reference/latest/resources/resource-configuration/schema-handling/activation/[activation support] with richer set of activation states and mappings
...
* Introducing concept of xref:/midpoint/reference/latest/roles-policies/roles-and-policies-configuration/[inducement] as a generalization of the user-account assignment concept

* xref:/midpoint/guides/admin-gui-user-guide/#keeping-metadata-for-all-objects-creation-modification-approvals[Keeping metadata for all objects] (creation, modification, approvals)

* More xref:/midpoint/reference/latest/roles-policies/assignment/configuration/[expression variables] to support complex RBAC assignment/inducement structures and dynamic roles

* Improved internal resource caching

* Improved import overwrite operation

* Ability to use dynamic expression in xref:/midpoint/reference/latest/resources/resource-configuration/[provisioning script] arguments

* Reconciliation xref:/midpoint/reference/latest/resources/resource-configuration/[provisioning scripts]

* Introducing xref:/midpoint/reference/latest/concepts/matching-rules/[matching rules] which means a better support for case-insensitive resource attributes (especially identifiers)

* Option not to ignore the source attribute when using simulated activation

* Improved handling of xref:/midpoint/reference/latest/resources/resource-configuration/protected-accounts/[protected accounts]

* Improved handling of tolerant attribute values using patterns (regexp)

* Ability to limit xref:/midpoint/reference/latest/expressions/mappings/[inbound mappings to a specific channel]

* xref:/midpoint/reference/latest/concepts/clockwork/model-context/serialization/[XML-based synchronization context serialization] to support seamless upgrades of running processes

* Built-in xref:/midpoint/reference/latest/upgrade/upgrade-guide/[object migration capability] for easier system upgrades and data model migrations

* xref:/midpoint/reference/latest/deployment/removing-obsolete-information/[Cleanup task] to automatically clean up old data from the system and make the data store sustainable
...
* Include mechanism for xref:/midpoint/reference/latest/expressions/object-template/[object templates]

* Resource-specific assignment enforcement policies

* New xref:/midpoint/reference/latest/synchronization/projection-policy/["relative" assignment enforcement policy]

* Configurable xref:/midpoint/reference/latest/synchronization/projection-policy/[legalization] of accounts that are violating assignment policy
...
* xref:/midpoint/reference/latest/misc/notifications/[Significantly improved notifications]
