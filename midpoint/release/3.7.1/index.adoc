...
* Improved handling of xref:/midpoint/reference/latest/expressions/function-libraries/[function libraries]
...
* URLs used by xref:/midpoint/reference/latest/deployment/stand-alone-deployment/[Stand-Alone Deployment] were changed to match the URLs used by Tomcat-based deployments of midPoint 3.6 and earlier.
...
Existing password reset configuration still works, but it will be replace by xref:/midpoint/reference/latest/security/credentials/password-reset/new-configuration/[new password reset configuration] in the future.
