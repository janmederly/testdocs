...
** xref:/midpoint/reference/latest/admin-gui/admin-gui-config/[Custom actions for object lists] ("user" tasks that can be launched from GUI)

** xref:/midpoint/reference/latest/security/authentication/post-authentication-configuration/[Custom pre-registration form]
...
** xref:/midpoint/reference/latest/concepts/relation/relation-configuration/[Relation Configuration]

** xref:/midpoint/reference/latest/deployment/service-account-management/[Service Account Management]

** Minor expression evaluation improvements

** Support for `subtype` in assignment and inducement

** Minor improvements to xref:/midpoint/reference/latest/expressions/expressions/script/functions/midpoint/[midPoint script libraries]


* Security improvements

** Separate xref:/midpoint/reference/latest/security/authorization/configuration/[authorizations] for _get_ and _search_ operations.

** xref:/midpoint/reference/latest/deployment/multitenancy/[Multitenancy] authorizations improvements

** xref:/midpoint/reference/latest/security/authorization/configuration/[Authorization zone of control]
...
You can use new `verify` command for xref:/midpoint/reference/latest/deployment/ninja/[Ninja] command-line tool to check your deployment.
...
====
MidPoint deployment method has changed in midPoint release 3.7. xref:/midpoint/reference/latest/deployment/stand-alone-deployment/[Stand-alone deployment] is now the default deployment method.
...
However as the data model was extended in 3.9 the database schema needs to be upgraded using the xref:/midpoint/reference/latest/upgrade/database-schema-upgrade/[usual mechanism]. There were also other changes that may affect some deployments:
...
The same refresh tasks that are used for xref:/midpoint/reference/latest/resources/manual/configuration/[manual resources] can be used as a lightweight replacement to clean up the shadows without reconciliation.
