...
** Support for xref:/midpoint/reference/latest/resources/multiaccounts/[multiple equivalent accounts] on a resource (inbound)

** xref:/midpoint/reference/latest/tasks/thresholds/[Thresholds and limits] (experimental)
...
** xref:/midpoint/reference/latest/admin-gui/sections-object-details/[Sections (virtual containers) in object details]
...
** xref:/midpoint/reference/latest/expressions/expressions/profiles/[Expression Profiles] (limited implementation)

** Request identifier in the audit log

** Numerous security fixes from the EU-FOSSA bug bounty (see xref:/midpoint/reference/latest/security/advisories/[Security Advisories])
...
* SOAP-based xref:/midpoint/reference/latest/legacy/soap/[IDM Model Web Service Interface] is deprecated.
...
* The xref:/midpoint/reference/latest/legacy/jaspersoft-studio/[integration of Jaspersoft Studio for midPoint (a.k.a. "Jasper plugin")] will not work with midPoint 4.0. The reporting web service was changed and the plugin was not yet adapted to that change.
...
*Stand-alone deployment is default and recommended deployment option*. See xref:/midpoint/reference/latest/deployment/stand-alone-deployment/[Stand-Alone Deployment] for more details.
...
The usual SQL scripts for xref:/midpoint/reference/latest/upgrade/database-schema-upgrade/[database schema upgrade] are provided.
...
Those were marked "for removal" in midPoint 3.9. Our xref:/midpoint/reference/latest/deployment/ninja/[Ninja] tool can be used to detect the use of those elements even in midPoint 3.9. The "ninja" should be used to audit your use of deprecated data items before attempting to upgrade to midPoint 4.0.
...
* *Element iteration in object template was renamed to iterationSpecification.* This change was needed due to major changes in midPoint object type hierarchy, somehow related to xref:/midpoint/reference/latest/schema/archetypes/[archetypes] functionality.
...
Therefore midPoint 3.9 that was just upgraded to 4.0 will have null values for `primaryIdentifierValue`. Those values should be computed and stored by using xref:/midpoint/reference/latest/tasks/shadow-refresh-task/[shadow refresh task].
...
* *Element account* can no longer be used as top-level element for xref:/midpoint/reference/latest/resources/shadow/[shadow objects]. Element `shadow` should be used instead.
...
See xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[Inbound Mapping] page for the details.

* Special authorization is needed to run reports (authorization-model-3#runReport).
Access to report web service requires this authorization as well (e.g. needed for access by xref:/midpoint/reference/latest/legacy/jaspersoft-studio/[Jaspersoft Studio]).
...
* There were numerous changes to the xref:/midpoint/reference/latest/interfaces/model-java/[IDM Model Interface] (Java).
...
* xref:/midpoint/reference/latest/legacy/soap/[IDM Model Web Service Interface] (SOAP) is deprecated.
