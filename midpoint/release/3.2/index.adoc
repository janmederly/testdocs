...
MidPoint 3.2 data model is essentially backwards compatible with midPoint 3.1.1. However as the data model was extended in 3.2 the database schema needs to be upgraded using the xref:/midpoint/reference/latest/upgrade/database-schema-upgrade/[usual mechanism].
...
There is a separate page describing the migration: xref:/midpoint/reference/latest/upgrade/connectors/ldap-connector-migration/[LDAP Connector Migration]
