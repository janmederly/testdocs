...
** xref:/midpoint/reference/latest/expressions/expressions/script/xpath/[XPath version 2] (deprecated)
...
** Major improvement in the use of xref:/midpoint/reference/latest/roles-policies/policy-rules/[Policy Rules]

** xref:/midpoint/reference/latest/roles-policies/segregation-of-duties/[Role exclusion]: pruning of conflicting roles which can be used to implement xref:/midpoint/reference/latest/roles-policies/rbac/radio-button-roles/[Radio Button Roles]
...
** xref:/midpoint/reference/latest/roles-policies/roles-and-policies-configuration/[Idempotent roles]

** Major performance improvements for cases with many assignments

* Password improvements

** xref:/midpoint/reference/latest/security/credentials/password-storage-configuration/[Password hashing]

** Mail-based initialization of new accounts (for use with password hashing)

** Check expression in xref:/midpoint/reference/latest/security/credentials/password-policy/[Password Policy]

** Support for password minimal age in xref:/midpoint/reference/latest/security/credentials/password-policy/[Password Policy]

** Improved handling of readable and unreadable resource password values

* Mapping and expression improvements

** Specification of xref:/midpoint/reference/latest/expressions/mappings/[mapping] domain and range

** RunAs configuration for xref:/midpoint/reference/latest/expressions/expressions/[expressions]
...
** xref:/midpoint/reference/latest/repository/full-text-search/[Quasi-full-text search]
...
** xref:/midpoint/reference/latest/resources/manual/[Manual Resource and ITSM Integration] (partially implemented)

** xref:/midpoint/reference/latest/resources/multi-connector-resource/[Multi-Connector Resource] (partially implemented)

* Partial execution of IDM model computation that allow ability for lighter xref:/midpoint/reference/latest/tasks/recompute-task/[recompute tasks]

* xref:/midpoint/reference/latest/expressions/constants/[Constants]
...
** xref:/midpoint/reference/latest/interfaces/rest/authentication/[REST Authentication] improvements (proxy authenticaiton, security questions authentication)
...
* xref:/midpoint/reference/latest/deployment/syslog-logging/[Syslog Logging]
...
MidPoint 3.6.1 data model is essentially backwards compatible with both midPoint 3.5 and midPoint 3.5.1. However as the data model was extended in 3.6.1 the database schema needs to be upgraded using the xref:/midpoint/reference/latest/upgrade/database-schema-upgrade/[usual mechanism].
...
* The focus xref:/midpoint/reference/latest/concepts/object-lifecycle/[object lifecycle state] influences assignment lifecycle.
...
* xref:/midpoint/reference/latest/roles-policies/policy-rules/[Policy Rules] with multiple constrains are evaluated in such a way that logical AND operation is assumed between the constraints.
...
* Previous midPoint versions applied changes in attribute and association mappings even in weaker xref:/midpoint/reference/latest/synchronization/projection-policy/[assignment enforcement modes] (none, positive).
