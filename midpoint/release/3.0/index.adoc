...
The support for xref:/midpoint/reference/latest/resources/entitlements/[entitlements] allows to extend the capabilities to groups, teams, privileges, resource-side roles and similar concepts which is moving midPoint in the direction of privileged identity management (PIM).
MidPoint now allows seamless synchronization of these objects, therefore groups can be easily synchronized with midpoint xref:/midpoint/reference/latest/roles-policies/rbac/[roles] which in turn can be synchronized with groups on other resources or even complex technical roles defined on the resource side.
MidPoint goes beyond the tradition even in this aspect.
As midPoint really understands the xref:/midpoint/reference/latest/resources/entitlements/[entitlements] it can do very smart tricks.
...
This is all considerably generic therefore similar principle can be applied to synchronization of xref:/midpoint/reference/latest/org/organizational-structure/[organizational structure] as well.
And (as xref:/midpoint/introduction/approach/[expected]) all the midPoint principles can be applied to these new features.
Therefore xref:/midpoint/reference/latest/roles-policies/rbac/[RBAC roles] can be applied to simplify synchronization of organizational unit.
...
* New xref:/midpoint/reference/latest/expressions/expressions/[expression evaluators] for assignments and entitlement associations
...
* xref:/midpoint/reference/latest/misc/bulk/[Bulk actions (midPoint scripting language)]
...
* Extended xref:/midpoint/reference/latest/expressions/expressions/script/functions/[function libraries] for expressions
