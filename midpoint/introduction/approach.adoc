...
It is used to automatically figure out correct data types in xref:/midpoint/reference/latest/expressions/mappings/[mappings]. The schema is also used for automatic conversion of data types if they do not match (e.g. automatic conversion of date and time values).
...
MidPoint needs just correctly defined xref:/midpoint/reference/latest/expressions/mappings/[mapping time constraints] and then it automatically takes care of all the messy management of time and tasks.
...
MidPoint needs just correctly defined xref:/midpoint/reference/latest/resources/resource-configuration/schema-handling/activation/[activation mappings] to do this.
...
* The complex problem of xref:/midpoint/reference/latest/synchronization/introduction/[synchronization] is divided to several steps (inbound, template, outbound) that are more or less independent.

* Each of the xref:/midpoint/reference/latest/synchronization/introduction/[synchronization] steps is also using simpler mechanisms, such as xref:/midpoint/reference/latest/expressions/mappings/[mappings] which handle the transformation of attribute values.

* The processing of xref:/midpoint/reference/latest/resources/resource-configuration/synchronization/[synchronization actions] is independent from both steps and also mappings inside them.

* xref:/midpoint/reference/latest/expressions/mappings/[Mappings] are using xref:/midpoint/reference/latest/expressions/expressions/[expressions] to transform individual values.
...
The somehow extreme examples are perhaps xref:/midpoint/reference/latest/roles-policies/rbac/[meta-roles] and xref:/midpoint/reference/latest/roles-policies/assignment/[higher-order inducements]. Please have a look at xref:/midpoint/reference/latest/samples/story-tests/orgsync/[OrgSync Story Test] description to get an idea how complex configuration can be created by using just a handful of mechanisms.
...
* xref:/midpoint/reference/latest/expressions/[Mappings and Expressions] are used in many places in midpoint:
xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[Inbound Mapping], xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[Outbound Mapping], xref:/midpoint/reference/latest/expressions/object-template/[Object Template],
...
and we even use xref:/midpoint/reference/latest/expressions/expressions/[expressions inside expressions].

* xref:/midpoint/reference/latest/synchronization/introduction/[Synchronization] policy is xref:/midpoint/reference/latest/resources/resource-configuration/synchronization/[configured] just once.
Then the same policy is used by all the xref:/midpoint/reference/latest/synchronization/flavors/[synchronization flavors] such as live synchronization, reconciliation or discovery.
It does not matter _how_ a change was discovered.
The reaction will always be consistent.

* xref:/midpoint/reference/latest/roles-policies/rbac/[Roles] and xref:/midpoint/reference/latest/expressions/object-template/[object templates] can be applied to numerous types of objects, not just users.
E.g. we can have roles that are applied to xref:/midpoint/reference/latest/org/organizational-structure/[organizational units] or even to roles itself (creating meta-roles).

* The principle of xref:/midpoint/reference/latest/expressions/mappings/outbound-mapping/[outbound mappings] is used both in xref:/midpoint/reference/latest/resources/resource-configuration/[resource definitions] and in xref:/midpoint/reference/latest/roles-policies/rbac/[roles]. The principles are the same.

* xref:/midpoint/reference/latest/org/organizational-structure/[Organizational structure] can be reused for many purposes: formal functional organizational structure of a company (divisions, sections), project-based structure, ad-hoc teams, multi-tenancy, even for a simple groupings of objects for the purpose of delegated administration.
Or all of that at once.
The same principles are used.

* xref:/midpoint/devel/prism/concepts/deltas/[Deltas] are used mostly for normal computation of changes (e.g. in xref:/midpoint/reference/latest/expressions/mappings/[mappings]). But they are also stored in the role requests that wait for an approval.
Deltas are stored in xref:/midpoint/reference/latest/resources/shadow/[shadows] while they wait for a target resource to come back online.
Deltas are used to re-start an operation after xref:/midpoint/reference/latest/synchronization/consistency/[resolving an incosistency].

* xref:/midpoint/reference/latest/roles-policies/rbac/[Roles] are used mostly for provisioning.
...
And there is one more reuse: the xref:/midpoint/reference/latest/org/organizational-structure/[organizational units] are considered to be also roles.
...
* MidPoint is using *xref:/midpoint/reference/latest/concepts/relativity/[relative] change model*.
...
When midPoint finds out that there is a problem it tries to xref:/midpoint/reference/latest/synchronization/consistency/[fix it automatically]. E.g. if midPoint tries to modify an account which is gone then midPoint will try to re-create it.
...
* One of the curial points in midPoint design is that midPoint clearly understand the difference between *policy* (what _should be_) and *reality* (what _is_). It has xref:/midpoint/reference/latest/roles-policies/assignment/assigning-vs-linking/[doubled mechanisms] to efficiently handle the distinction.
...
The xref:/midpoint/reference/latest/roles-policies/assignment/[assignments] can be used to set the policy, the xref:/midpoint/reference/latest/roles-policies/assignment/assigning-vs-linking/[links] and xref:/midpoint/reference/latest/resources/shadow/[shadows] track the reality and xref:/midpoint/reference/latest/synchronization/introduction/[synchronization] is used to align them.
...
MidPoint is based on the idea of the xref:/midpoint/reference/latest/schema/[unified data model] which in turn is based on link:http://en.wikipedia.org/wiki/Single_Source_of_Truth[Single Source of Truth principle]. The data model of midPoint is defined as an abstract schema.
