...
* *xref:/midpoint/reference/latest/roles-policies/rbac/[Role-based access control (RBAC)]*: midPoint can automatically compute user privileges based on his membership in roles.
xref:/midpoint/reference/latest/roles-policies/rbac/[MidPoint RBAC] model is one of the most powerful models in the entire IDM field.
...
* Management of *xref:/midpoint/reference/latest/org/organizational-structure/[organizational structure]* and its synchronization to other systems.
...
MidPoint can be configured with a set of xref:/midpoint/reference/latest/expressions/[rules and scripts] that is used to process the data.
E.g. the rules may take the field "organization unit" from the HR record and used that to determine which xref:/midpoint/reference/latest/org/organizational-structure/[organizational unit] the user belongs to and which xref:/midpoint/reference/latest/roles-policies/rbac/[business roles] should this user have.
This logic is different in each organization and midPoint is built to be xref:/midpoint/reference/latest/expressions/[easily customizable].
...
This is usually computed from the xref:/midpoint/reference/latest/roles-policies/rbac/[roles] that the user has.
Account attributes and xref:/midpoint/reference/latest/resources/entitlements/[entitlements] are computed as well, e.g. the list of groups that user should belong to.
...
MidPoint applies xref:/midpoint/reference/latest/roles-policies/policy-rules/[policies] to those data.
MidPoint can enforce xref:/midpoint/reference/latest/roles-policies/segregation-of-duties/[segregation of duties] policy.
It can support xref:/midpoint/reference/latest/concepts/object-lifecycle/[object lifecycle] policies.
MidPoint can govern xref:/midpoint/reference/latest/org/organizational-structure/[organizational structure]. Those are business aspects of identity management.
...
The system is using an xref:/midpoint/reference/latest/resources/connid/[ConnId framework] as a mechanism to interact with other systems (resources).
