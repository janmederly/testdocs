...
MidPoint is designed and build with xref:/midpoint/reference/latest/concepts/relativity/[relativity] in mind.
This means that midPoint always thinks in a notion of relative changes (xref:/midpoint/devel/prism/concepts/deltas/[deltas]). However, midPoint computational core (a.k.a xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[clockwork and projector]) often require knowledge of the absolute state of a resource object such as account or group.
...
Features such as xref:/midpoint/reference/latest/resources/provisioning-dependencies/[provisioning dependencies] or xref:/midpoint/reference/latest/synchronization/consistency/[consistency mechanism] forced us to do some compromises.
...
xref:/midpoint/reference/latest/expressions/[Mappings and expressions] also suffer from a similar problem.
While xref:/midpoint/reference/latest/expressions/mappings/mapping-relativity/[mappings are designed to be fully relative], current implementation is evaluating mappings too often.
...
This problem did not affect midPoint deployments too much when xref:/midpoint/reference/latest/resources/connid/[ConnId connectors] were the only mechanism to detect changes.
...
The plan is to improve implementation of xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[Clockwork and Projector] to support completely relativistic mode.
...
It is also the right thing to do to align the implementation with the original desig and the ideal of xref:/midpoint/reference/latest/concepts/relativity/[relativity].
...
* xref:/midpoint/reference/latest/expressions/mappings/mapping-relativity/[Mapping Relativity]
