...
MidPoint has strong xref:/midpoint/reference/latest/synchronization/introduction/[synchronization] features that includes ability to automatically xref:/midpoint/reference/latest/synchronization/correlation-and-confirmation-expressions/[correlate] identities.
xref:/midpoint/reference/latest/synchronization/correlation-and-confirmation-expressions/[Correlation expression] is a simple but very powerful mechanism to correlate identities when reliable correlation identifier is present.
...
Actually, first step towards improved correlation is already implemented: xref:/midpoint/reference/latest/synchronization/synchronization-sorter/[Synchronization Sorter]. Sorter is sophisticated expression that can be used to implement completely custom identity correlation mechanism.
...
For example there is a `disputed` xref:/midpoint/reference/latest/synchronization/situations/[synchronization situation.]
...
Once again, such an extension was anticipated from the beginning and this was one of the reasons that midPoint has xref:/midpoint/reference/latest/resources/shadow/[shadow objects]. In that case xref:/midpoint/reference/latest/synchronization/correlation-and-confirmation-expressions/[correlation expressions] or xref:/midpoint/reference/latest/synchronization/synchronization-sorter/[sorter] can be used to determine the candidate matches.
In case of multiple matches (or a single low-confidence match) the account will end up in `disputed` situation.
Candidate matches will be recorded in the xref:/midpoint/reference/latest/resources/shadow/[shadow object]. MidPoint user interface can be extended to look for disputed correlation cases and present them to operator.
...
Cases are planned to support many midPoint features where manual work is required from xref:/midpoint/reference/latest/cases/approval/[approvals] and xref:/midpoint/reference/latest/resources/manual/[manual resources] to xref:/midpoint/features/planned/remediation/[remediation]. Manual correlation is just another reuse of the same principle.
