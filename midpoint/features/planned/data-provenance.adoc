...
Currently midPoint can provide information about data provenance by using xref:/midpoint/reference/latest/security/audit/[audit trail]. But audit records are a time-organized data that may be warehoused, summarized or archived.
...
* The resource where that particular data item came from (if set by xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound mapping])
