...
Luckily, midPoint is also based on concept of xref:/midpoint/reference/latest/concepts/relativity/[relative changes], therefore it can be quite forgiving if the data are not entirely up-to-date.
...
Previous work on midPoint motivated us to start experimenting with xref:/midpoint/reference/latest/resources/attribute-caching/[attribute caching] some time ago.
...
MidPoint will process the messages, internally invoking notifyChange() operation which is a part of xref:/midpoint/reference/latest/interfaces/model-java/[IDM Model Interface]. This operation is used to let midPoint know that a change has occurred on the resource - which is exactly the meaning of the message.
This interaction can handle synchronization (inbound create, update and delete changes).
Read and search operations will be handled by xref:/midpoint/reference/latest/resources/attribute-caching/[attribute caching]. Outbound write operations (create, update and delete) will not be supported.
...
Target messaging-based resource may use a new special built-in midPoint connector in a similar way that xref:/midpoint/reference/latest/resources/manual/[manual resources] are using.
...
The later case (updating account) means that we may use midPoint xref:/midpoint/reference/latest/expressions/mappings/inbound-mapping/[inbound mappings] that are easily configurable.
...
Similar xref:/midpoint/reference/latest/resources/multi-connector-resource/[multi-connector] functionality was achieved for xref:/midpoint/reference/latest/resources/manual/configuration/[semi-manual resources] therefore it may also be possible in this case.
...
The xref:/midpoint/reference/latest/resources/multi-connector-resource/[multi-connector] functionality may be one of the solutions here.
...
xref:/midpoint/reference/latest/resources/resource-schema/[Resource schema] needs to be configured manually in midPoint, as there is no way for automatic schema discovery in this case.
But this is not a big problem (once again it is similar to manual connectors).

While midPoint is built with xref:/midpoint/reference/latest/concepts/relativity/[relativity] in mind, this relativity is not complete.
...
* xref:/midpoint/reference/latest/resources/attribute-caching/[Attribute Caching]
