...
It is common that identity management systems govern complex xref:/midpoint/reference/latest/cases/approval/[processes] based on sophisticated xref:/midpoint/reference/latest/roles-policies/policy-rules/[policies and rules].
The processes are built to maintain identity-related structures such as xref:/midpoint/reference/latest/roles-policies/rbac/[role-based access control (RBAC)] structures, xref:/midpoint/reference/latest/org/organizational-structure/[organizational hierarchies], project-oriented structures, self-managed organizations and all the other kinds of identity-related structures.
...
One such a mechanism is already planned for midPoint development: xref:/midpoint/reference/latest/admin-gui/collections-views/[Object Collections and Views].
...
This compliance reporting gets even more interesting when it is coupled with xref:/midpoint/reference/latest/roles-policies/policy-rules/[policy rules].
...
In addition to that compliance reporting is firmly based on xref:/midpoint/reference/latest/roles-policies/policy-rules/[policy rules] which form a base of a very powerful midPoint feature.
...
It is designed to fit together another planned midPoint feature: dynamic and xref:/midpoint/reference/latest/admin-gui/dashboards/[customizable dashboard].
...
xref:/midpoint/reference/latest/synchronization/introduction/[Synchronization] takes place almost all the time.
...
Some such situations are handled automatically by xref:/midpoint/reference/latest/synchronization/introduction/[synchronization] mechanism.
...
We can also simply mark the violating xref:/midpoint/reference/latest/resources/shadow/[shadow object] with a specific policy situation marker.
...
But there are fine-grain in midPoint: recomputation of attributes values, application of roles, attribute-level reconciliation, processing of xref:/midpoint/reference/latest/resources/entitlements/[entitlements] and so on.
...
MidPoint already has xref:/midpoint/reference/latest/misc/reports/[reporting features] that can be used to create similar reports.
