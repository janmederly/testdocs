...
Therefore midPoint is completely xref:/midpoint/reference/latest/cases/workflowless/[workflowless] now.

Even though midPoint has xref:/midpoint/reference/latest/cases/approval/policy-based-approvals/[policy-based approvals], there is still a need for a _processes._ Maybe there is an enrollment process for a new employee.
...
This ability is used for internal xref:/midpoint/reference/latest/cases/approval/policy-based-approvals/[approvals] in midPoint.
...
The plugin can integrate into midPoint processes using xref:/midpoint/reference/latest/concepts/clockwork/clockwork-and-projector/[clockwork] hooks.
...
Therefore the plugin has either to be bundled into midPoint or there must be a custom build by using xref:/midpoint/reference/latest/deployment/maven-overlay-project/[overlay project].
...
We also need to think about security (e.g. xref:/midpoint/reference/latest/expressions/expressions/profiles/[Expression Profiles]).
...
* xref:/midpoint/reference/latest/cases/workflowless/[Workflowless]

* xref:/midpoint/reference/latest/cases/approval/policy-based-approvals/[Policy-based approvals]