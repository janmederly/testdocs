...
For this we can reuse existing xref:/midpoint/reference/latest/concepts/activation/[assignment validity constraints.] The protection scopes may be hierarchical: one protection scope being subset of another protection scope.
xref:/midpoint/reference/latest/roles-policies/rbac/[Role hierarchy] can be reused to model this behavior.
And so on.
...
Protection scopes and even individual assignments might be made to be part of xref:/midpoint/reference/latest/roles-policies/certification/[certification] campaigns.
The protection scopes are very likely to evolve as the legislation evolves.
Therefore the xref:/midpoint/reference/latest/roles-policies/role-lifecycle/[role lifecycle management] may be used to manage the evolution of lawful bases as well.
