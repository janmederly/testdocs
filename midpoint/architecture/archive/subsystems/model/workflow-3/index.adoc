...
* xref:/midpoint/reference/latest/cases/workflow-3/[general information about workflow feature],
...
There is a fundamental mechanism for carrying out and monitoring long-running, asynchronous operations in midPoint (such as operations that require human approval): xref:/midpoint/reference/latest/tasks/task-manager/[tasks].
...
For dumb processes, these tasks are active (in RUNNABLE state), waking each xref:/midpoint/reference/latest/cases/workflow-3/legacy-pre-3-5-workflow-configuration/[processCheckInterval] secods, and actively checking the wf process instance state.
