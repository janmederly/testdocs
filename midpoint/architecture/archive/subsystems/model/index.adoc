...
However, it is also using other parts of the xref:/midpoint/reference/latest/schema/[Data Model], especially the dynamic xref:/midpoint/reference/latest/resources/resource-schema/[Resource Schema]. IDM model is interpreting the schema and also evaluating the expressions specified in the xref:/midpoint/reference/latest/resources/resource-configuration/schema-handling/[Resource Schema Handling] part.

One of the most important data model concepts that is processed by IDM model is a concept of xref:/midpoint/reference/latest/roles-policies/assignment/[Assignment].
...
It is only processing Inbound and Outbound Expressions and handles xref:/midpoint/reference/latest/synchronization/examples/[Synchronization Examples]. RBAC or other "advanced" mechanisms are not yet implemented.
...
It is designed (see xref:/midpoint/reference/latest/roles-policies/rbac/[Advanced Hybrid RBAC]) and the implementation awaits prioritization in the xref:/midpoint/roadmap/[Roadmap].


=== IDM Model and Busines Logic

xref:/midpoint/reference/latest/legacy/soap/[IDM Model Web Service Interface] is probably an interface that will often used by business logic to invoke provisioning operations.
