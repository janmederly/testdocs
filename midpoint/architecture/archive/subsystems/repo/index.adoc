...
The input and output data to the xref:/midpoint/architecture/archive/subsystems/repo/identity-repository-interface/[interface of the repository subsystem] are objects from the xref:/midpoint/reference/latest/schema/[Data Model]. The data model objects may be represented in XML and the repository may take advantage of that.
...
The repository subsystem is storing and retrieving data objects that belong to the xref:/midpoint/reference/latest/schema/[Data Model]. However, the repository subsystem is just storing and retrieving the data objects, it does not implement any special logic or special behavior.
...
The majority of the objects in the standard xref:/midpoint/reference/latest/schema/[Data Model] have a special `extension` XML element.
...
There is the xref:/midpoint/reference/latest/schema/[Generic Object] type which is designed especially for the purpose of holding custom data.
