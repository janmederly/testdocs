...
It stores objects that are defined by the xref:/midpoint/reference/latest/schema/[Data Model]. It allows to read, create, update and delete objects (CRUD), it also provides operations for searching for the object and support references between objects.
