...
The Provisioning Subsystem is exposing the xref:/midpoint/reference/latest/resources/resource-schema/[Resource Schema] which describes types and content of all resource objects for each resource instance.
...
Shadow cache handles Resource Object Shadows (see xref:/midpoint/reference/latest/resources/shadow/[Shadow Objects]). These "shadows" are stored partially in repository and partially on the resource.
...
See xref:/midpoint/reference/latest/resources/shadow/[Shadow Objects] for more details.
...
Schema processor from the xref:/midpoint/architecture/archive/subsystems/infra/[Infrastructure Subsystem] is used to handle the dynamic xref:/midpoint/reference/latest/resources/resource-schema/[Resource Schema].
It is used by almost all provisioning layers.

== Data Structures

The Provisioning Subsystem is responsible for managing the following objects from the xref:/midpoint/reference/latest/schema/[Data Model]:
...
* Resource objects that conform to a (dynamic) xref:/midpoint/reference/latest/resources/resource-schema/[Resource Schema] for a particular Resource
...
Dynamic objects (from xref:/midpoint/reference/latest/resources/resource-schema/[Resource Schema]) are not stored directly.
If they are stored, they take form of xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/[Resource Object Shadow]. See xref:/midpoint/reference/latest/resources/shadow/[Shadow Objects] for more details.
