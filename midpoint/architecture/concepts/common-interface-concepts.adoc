...
All the interfaces provide manipulation of the identity objects (objects from the xref:/midpoint/reference/latest/schema/[data model]), therefore it is quite expected that all the interfaces will provide similar semantics.
...
* xref:/midpoint/reference/latest/interfaces/model-java/[IDM Model Interface] (Model API)
...
The implementation may be able to _resolve_ xref:/midpoint/reference/latest/schema/object-references/[object references]. The client may request that the resulting object should be returned as xref:/midpoint/reference/latest/schema/object-references/[composite], instead of a plain object with references.
...
They return all objects of selected type (`list`) or objects of selected type that match a specified xref:/midpoint/reference/latest/concepts/query/xml-query-language/[filter].
...
* search - returning subset of objects with flexible search criteria (xref:/midpoint/reference/latest/concepts/query/xml-query-language/[filter])
...
* xref:/midpoint/reference/latest/concepts/query/xml-query-language/[Query Language]
