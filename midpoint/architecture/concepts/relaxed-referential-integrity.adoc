...
MidPoint xref:/midpoint/reference/latest/schema/[data model] contains a lot of xref:/midpoint/reference/latest/schema/object-references/[object references]. Object references are inherently uni-directional.
...
And for the rare case when we really want to check for references the check may be implemented by xref:/midpoint/reference/latest/roles-policies/policy-rules/[policy rules] (but there are limitations, see below).
...
* Do not delete xref:/midpoint/reference/latest/resources/shadow/[shadows] if not needed.
...
And if shadow disappears as a reaction to a deleted account, proper setup of xref:/midpoint/reference/latest/synchronization/introduction/[synchronization mechanisms] will make sure that the links are properly removed.

* Apply proper practices to deal with xref:/midpoint/reference/latest/concepts/object-lifecycle/[lifecycle] of objects that are target of assignments.
...
xref:/midpoint/reference/latest/misc/bulk/[Bulk actions] are often a very useful tool in such cases.

* There are cases when we want to prohibit deletion of a target in case that it has assignees.
xref:/midpoint/reference/latest/roles-policies/policy-rules/[Policy rules] are mechanism that is designed to take care of that.
However, there still may be some limitations.
Please see the xref:/midpoint/reference/latest/roles-policies/policy-rules/[Policy Rules] page for the details.
...
* xref:/midpoint/reference/latest/schema/object-references/[Object References]
