...
The majority of common IDM scenarios can be _configured_ in midPoint without any need for programming (except perhaps for one-line scripting expressions in xref:/midpoint/reference/latest/expressions/mappings/[mappings]).
...
It is a component through which all activities pass, therefore it is an ideal place to enforce policies, fill-in missing data, xref:/midpoint/reference/latest/expressions/[map attributes and properties], govern processes and do all the identity management logic.
...
The mechanism of the xref:/midpoint/reference/latest/roles-policies/rbac/[MidPoint Role-Based Access Control model (RBAC)] is here.
...
Provisioning system maintains xref:/midpoint/reference/latest/resources/shadow/[shadow objects], which are prepresentations of accounts, groups and similar objects residing on source and target systems.
...
Provisioning system executes the operations on source and target systems (known as "resources") by using _identity connectors_. xref:/midpoint/reference/latest/resources/connid/[ConnId connector framework] is used to manage the connectors.
...
Infrastructure subsystem is also a place where the xref:/midpoint/reference/latest/schema/[Data Model] is materialized.
...
Main page: xref:/midpoint/reference/latest/schema/[Data Model]
...
* xref:/midpoint/reference/latest/concepts/data-representation-formats/['Objects, XML, JSON and others']
