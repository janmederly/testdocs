...
Our xref:/midpoint/reference/latest/deployment/ninja/[Ninja] tool was received in a *positive way*.
...
MidPoint has a xref:/midpoint/reference/latest/deployment/stand-alone-deployment/[Stand-Alone Deployment] for several years already - and the *stand-alone deployment is generally preferred by the community*.
...
But what came as a surprise to me was that xref:/midpoint/reference/latest/roles-policies/metaroles/[Metaroles] were mentioned quite often.
