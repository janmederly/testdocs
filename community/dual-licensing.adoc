...
*Therefore no CLAs will be necessary to contribute to translations and samples.* It is also likely that some testing scenarios (e.g. xref:/midpoint/reference/latest/samples/story-tests/[Story Tests]) will be separated from midPoint in the future.
