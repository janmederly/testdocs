...
This is likely to be xref:/midpoint/reference/latest/samples/story-tests/["story test"]. We expect quite a few positive test cases and maybe handful of negative test cases.
...
* Feature configuration page (usually under xref:/midpoint/reference/latest/[Administration and Configuration Guide]): Page that describes the details of feature mechanics, configuration, usage and so on.
...
Please, follow our xref:/midpoint/reference/latest/diag/creating-a-bug-report/[bug report guidelines] when reporting a bug.
